...
async function run() {
  ...
  try {
    ...
    for (const sale of existingSales.data) {
      ...
      console.log(`🗑️ Cleaned existing sales ${sale.id}`);
    }
    ...
    const salesResp = await axios.post(`${BASE_URL}/api/forms/daily-sales/v3`, {
      ...
      notes: "Smoke test — Phase 2 locked"
    });
    salesId = salesResp.data.id;
    console.log("✓ Daily Sales V2 created →", salesId);

    // 2. Daily Stock V2 → auto Shopping List V2
    ...
    console.log("✓ JSON valid — contains sales, stock, variance, shoppingList");
    ...
    console.log(`✓ Reports list contains today's report (${list.data.reports.length} total)`);

    console.log("\n🎉 PHASE 2 IS 100% LOCKED — ALL TESTS PASSED");
