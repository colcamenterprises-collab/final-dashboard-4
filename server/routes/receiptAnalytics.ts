...
async function resolveWindow(q: Record<string, unknown>): Promise<Window | null> {
  ...
  if (fd && ft && td && tt) {
    ...
    return {
      mode: "exact_datetime",
      shiftIds: shifts.rows.map((r) => String(r.id)),
      fromDate: fd,
      toDate: td,
      windowStart: start.toFormat("yyyy-MM-dd HH:mm:ss"),
      windowEnd: end.toFormat("yyyy-MM-dd HH:mm:ss"),
  ...
  if (mode === "current_shift") {
    const r = await pool.query(`SELECT id,opened_at,COALESCE(closed_at,NOW()) ended_at FROM pos_shifts WHERE status='open' ORDER BY opened_at DESC LIMIT 1`);
    if (!r.rowCount) return null;
    const x = r.rows[0];
    const d = DateTime.fromJSDate(new Date(x.opened_at)).setZone(TZ).toISODate()!;
  ...
  if (mode === "custom" || q.shiftStartDate || q.from) {
    ...
    if (!r.rowCount) return null;
    return {
      mode: from === to ? "shift_date" : "custom_shift_range",
      shiftIds: r.rows.map((x) => String(x.id)),
      fromDate: from,
      toDate: to,
      windowStart: bkkStamp(r.rows[0].opened_at),
      windowEnd: bkkStamp(r.rows[r.rows.length - 1].ended_at),
    };
  }

  const limit = Math.min(Math.max(parseInt(s(q.limit), 10) || 1, 1), 90);
  const r = await pool.query(`SELECT id,opened_at,closed_at FROM pos_shifts WHERE status='closed' ORDER BY opened_at DESC LIMIT $1`, [limit]);
  if (!r.rowCount) return null;
  const rows = [...r.rows].reverse();
  const from = DateTime.fromJSDate(new Date(rows[0].opened_at)).setZone(TZ).toISODate()!;
  const to = DateTime.fromJSDate(new Date(rows[rows.length - 1].opened_at)).setZone(TZ).toISODate()!;
  return {
    mode: limit > 1 ? `last_${limit}_shifts` : "last_completed_shift",
    shiftIds: rows.map((x) => String(x.id)),
    fromDate: from,
    toDate: to,
    windowStart: bkkStamp(rows[0].opened_at),
    windowEnd: bkkStamp(rows[rows.length - 1].closed_at),
  };
}

function receiptScope(w: Window) {
  if (w.mode === "exact_datetime") {
    return { sql: `o.created_at >= $1::timestamptz AND o.created_at < $2::timestamptz`, params: [w.startUtc, w.endUtc] };
  }
  return {
    sql: `(o.pos_shift_id=ANY($1::uuid[]) OR (o.pos_shift_id IS NULL AND EXISTS(SELECT 1 FROM pos_shifts ps WHERE ps.id=ANY($1::uuid[]) AND o.created_at>=ps.opened_at AND o.created_at<=COALESCE(ps.closed_at,NOW()))))`,
    params: [w.shiftIds],
  };
}
...
router.get("/shift-review", async (req, res) => {
  try {
    ...
    if (!w || w.shiftIds.length < 1) return res.json({ ok: false, source: "sbb_pos_core", message: "No POS shift found", rows: [] });
    if (w.shiftIds.length !== 1) return res.status(400).json({ ok: false, error: "Select one shift for Shift Review" });
    const id = w.shiftIds[0];
    const sr = await pool.query(`SELECT id,staff_name,opened_at,closed_at,starting_float,closing_cash,cash_banked,status,(opened_at AT TIME ZONE 'Asia/Bangkok')::date::text shift_date FROM pos_shifts WHERE id=$1`, [id]);
    ...
    res.status(500).json({ ok: false, source: "sbb_pos_core", error: e.message });
  }
});
  ...
  try {
    ...
    if (!w) {
      return res.json({
        ok: false,
        source: "sbb_pos_core",
        blockers: [{ code: "NO_POS_SHIFTS", message: "No POS shifts found" }],
        ...
        topProducts: [], itemSales: [], topModifiers: [], componentSales: [], includedComponents: [], categoryMix: [], dailyTrend: [], hourlySales: [], paymentMix: [], receipts: [],
        filters: { from: "", to: "", mode: "", timezone: TZ },
      });
    }

    const sc = receiptScope(w);
    ...
    const [sumR, itemR, modR, compR, catR, hourR, payR, recR] = await Promise.all([
      ...
      pool.query(
        `WITH receipts AS(${base}),
         mods AS(SELECT order_item_id,COALESCE(SUM(price_delta*quantity),0) mod_total FROM ordering_order_item_modifiers GROUP BY order_item_id),
         sold AS(
           SELECT i.item_name_en name,COALESCE(i.source_sku,'') sku,${categoryCase} category,i.quantity,i.line_total,COALESCE(mods.mod_total,0) mod_total,
                  COALESCE(r.subtotal,r.total,0) receipt_subtotal,COALESCE(r.total,0) receipt_total
           FROM ordering_order_items i
           JOIN receipts r ON r.id=i.order_id
           LEFT JOIN mods ON mods.order_item_id=i.id
      ...
      pool.query(
        `WITH receipts AS(${base}) SELECT r.id,r.order_number,LEFT(r.ticket_number,3) receipt_number,r.order_mode,r.channel,r.payment_method,r.subtotal,GREATEST(COALESCE(r.subtotal,r.total,0)-COALESCE(r.total,0),0) discount_amount,r.total,r.created_at,(r.created_at AT TIME ZONE 'Asia/Bangkok')::timestamp bkk_created_at,r.pos_shift_id,
         COALESCE((SELECT jsonb_agg(jsonb_build_object('id',i.id,'name',i.item_name_en,'quantity',i.quantity,'unitPrice',i.unit_price,'lineTotal',i.line_total,'isSetComponent',COALESCE(i.is_set_component,false),'modifiers',COALESCE((SELECT jsonb_agg(jsonb_build_object('group',m.modifier_group_name_en,'name',m.modifier_name_en,'priceDelta',m.price_delta,'quantity',m.quantity,'type',${modifierTypeCase})) FROM ordering_order_item_modifiers m WHERE m.order_item_id=i.id),'[]'::jsonb)) ORDER BY i.sort_order) FROM ordering_order_items i WHERE i.order_id=r.id),'[]'::jsonb) items
         FROM receipts r ORDER BY r.created_at DESC LIMIT 1000`,
        params,
      ),
    ]);
    ...
    const itemPctDenominator = Math.max(itemQty, 1);
    const modPctDenominator = Math.max(modQty, 1);
    const catQty = (name: string) => cats.find((x) => x.category === name)?.qtySold || 0;

    res.json({
      ok: true,
      source: "sbb_pos_core",
      schemaVersion: "item-sales-v1",
      summary: {
        grossSales: n(sr.gross_sales),
        receiptCount: n(sr.receipt_count),
        averageReceiptValue: n(sr.avg_receipt),
        lineItemCount: itemQty,
        modifierCount: modQty,
        burgersSold: catQty("Burgers"),
        friesSold: catQty("Fries"),
        drinksSold: catQty("Drinks"),
        chickenSold: catQty("Chicken"),
      },
      topProducts: items.slice(0, 50).map((x) => ({ name: x.name, sku: x.sku, category: x.category, qtySold: n(x.qty_sold), revenue: n(x.net_sales), pctOfTotal: Number((n(x.qty_sold) / itemPctDenominator * 100).toFixed(1)) })),
      ...
      topModifiers: mods.map((x) => ({ name: `${x.group_name}: ${x.name}`, qtySold: n(x.qty_sold), pctOfTotal: Number((n(x.qty_sold) / modPctDenominator * 100).toFixed(1)) })),
