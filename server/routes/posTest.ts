...
const displayTicket = (value: unknown) => String(value || "").split("-")[0].slice(0, 3);
...
router.post("/:id/movements", async (req, res) => {
  ...
  try {
    ...
    const open = await client.query(`SELECT id FROM public.pos_test_shifts WHERE id=$1 AND status='open' FOR UPDATE`, [req.params.id]);
    if (!open.rowCount) {
...
router.post("/:id/close", async (req, res) => {
  ...
  try {
    ...
    const cashSalesResult = await client.query(`SELECT COALESCE(SUM(total),0) AS total FROM public.pos_test_orders WHERE shift_id=$1 AND payment_method='cash' AND status <> 'cancelled'`, [current.id]);
    const cashSales = numberValue(cashSalesResult.rows[0]?.total);
...
router.post("/orders", async (req, res) => {
  ...
  try {
    await client.query("BEGIN");
    const shift = (await client.query(`SELECT * FROM public.pos_test_shifts WHERE status='open' ORDER BY opened_at DESC LIMIT 1 FOR UPDATE`)).rows[0];
    ...
    const ticket = String(((sequence - 1) % 999) + 1).padStart(3, "0");
    const storedTicket = `${ticket}-${String(sequence).padStart(6, "0")}`;
    let subtotal = 0;
    ...
    for (const line of input.items) {
      ...
      const applicableGroups = (await client.query(
        `SELECT g.id,g.name_en,g.selection_mode,g.min_selections,g.max_selections
           FROM public.ordering_modifier_groups g
          WHERE g.is_active
            AND (g.menu_item_id=$1 OR EXISTS(SELECT 1 FROM public.ordering_modifier_group_items a WHERE a.modifier_group_id=g.id AND a.menu_item_id=$1))`,
        [item.id],
      )).rows;
      for (const group of applicableGroups) {
        const selectedCount = selectedModifiers.filter((modifier: any) => modifier.modifier_group_id === group.id).length;
        const minimum = Math.max(0, numberValue(group.min_selections));
        const configuredMax = group.max_selections == null ? null : Math.max(0, numberValue(group.max_selections));
        const maximum = group.selection_mode === "single" ? 1 : configuredMax;
        if (selectedCount < minimum) throw new Error(`${group.name_en} requires at least ${minimum} selection${minimum === 1 ? "" : "s"}`);
        if (maximum !== null && selectedCount > maximum) throw new Error(`${group.name_en} allows at most ${maximum} selection${maximum === 1 ? "" : "s"}`);
      }
    ...
    const created = (await client.query(
      ...
      [shift.id, storedTicket, mode, input.payment_method, grabOrderNumber, mode === "grab" ? customerName : null, mode === "grab" ? customerMobile : null, subtotal, discountAmount, total, JSON.stringify(payload)],
    )).rows[0];
    await client.query("COMMIT");
...
router.get("/orders/:id/receipt", async (req, res) => {
  try {
    ...
    res.json({ ok: true, source: SOURCE, test_mode: true, data: { ...row, ticket_number: displayTicket(row.ticket_number), receipt_number: displayTicket(row.ticket_number), items: payload.items || [] } });
  } catch (error: any) {
    fail(res, error.message, 500);
  }
});
...
router.get("/kitchen/orders", async (_req, res) => {
  try {
    ...
    const data = rows.map((row: any) => ({ ...row, ...(row.payload || {}), ticket_number: displayTicket(row.ticket_number), order_mode: row.order_mode, customer_name: row.customer_name }));
...
router.patch("/orders/:id/status", async (req, res) => {
  ...
  try {
    await client.query("BEGIN");
    const order = (await client.query(`SELECT * FROM public.pos_test_orders WHERE id=$1 FOR UPDATE`, [req.params.id])).rows[0];
    if (!order) {
      await client.query("ROLLBACK");
      return fail(res, "Test order not found", 404);
    }
    if (["completed", "cancelled"].includes(order.status)) {
      await client.query("ROLLBACK");
      return fail(res, "Finalized test orders cannot be changed", 409);
    }
    const shift = await client.query(`SELECT id FROM public.pos_test_shifts WHERE id=$1 AND status='open' FOR UPDATE`, [order.shift_id]);
    if (!shift.rowCount) {
      await client.query("ROLLBACK");
      return fail(res, "Test shift is closed", 409);
    }
    const updated = (await client.query(`UPDATE public.pos_test_orders SET status=$2,updated_at=NOW() WHERE id=$1 RETURNING *`, [req.params.id, status])).rows[0];
    await client.query("COMMIT");
    res.json({ ok: true, source: SOURCE, test_mode: true, data: { ...updated, ticket_number: displayTicket(updated.ticket_number) } });
  } catch (error: any) {
    await client.query("ROLLBACK");
    fail(res, error.message, 500);
  } finally {
...
router.get("/display/orders", async (_req, res) => {
  try {
    ...
    const data = rows.map((row: any) => ({ ...row, ticket_number: displayTicket(row.ticket_number) }));
    res.json({ ok: true, source: SOURCE, test_mode: true, data });
  } catch (error: any) {
    fail(res, error.message, 500);
  }
