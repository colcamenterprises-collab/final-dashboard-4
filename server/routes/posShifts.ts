...
async function ensureSchema() {
  await pool.query(`CREATE TABLE IF NOT EXISTS pos_shifts (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS pos_shift_movements (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS pos_one_open_shift_idx ON pos_shifts ((status)) WHERE status='open'`);
}

async function currentShift() {
  await ensureSchema();
  const result = await pool.query(`SELECT * FROM pos_shifts WHERE status='open' ORDER BY opened_at DESC LIMIT 1`);
...
router.get("/current", staffDevice, async (_req, res) => {
  try {
    ...
    const movements = shift ? (await pool.query(`SELECT * FROM pos_shift_movements WHERE shift_id=$1 ORDER BY created_at DESC`, [shift.id])).rows : [];
    const history = (await pool.query(`SELECT * FROM pos_shifts ORDER BY opened_at DESC LIMIT 20`)).rows;
...
router.post("/open", staffDevice, async (req, res) => {
  try {
    ...
    const result = await pool.query(`INSERT INTO pos_shifts(staff_name,starting_float,opened_by) VALUES($1,$2,$3) RETURNING *`, [staffName, startingFloat, actor]);
...
router.post("/:id/movements", staffDevice, async (req, res) => {
  try {
    ...
    const open = await pool.query(`SELECT id FROM pos_shifts WHERE id=$1 AND status='open'`, [req.params.id]);
    if (!open.rowCount) return fail(res, "Shift is not open", 409);
    const actor = (req as any).user?.username || (req as any).user?.id || null;
    const result = await pool.query(`INSERT INTO pos_shift_movements(shift_id,movement_type,amount,reason,created_by) VALUES($1,$2,$3,$4,$5) RETURNING *`, [req.params.id, movementType, amount, reason, actor]);
    res.status(201).json({ ok: true, source: "sbb_pos_shifts", data: result.rows[0] });
  } catch (error: any) {
    fail(res, error.message, 500);
  }
...
router.post("/:id/close", staffDevice, async (req, res) => {
  ...
  try {
    ...
    const current = (await client.query(`SELECT * FROM pos_shifts WHERE id=$1 AND status='open' FOR UPDATE`, [req.params.id])).rows[0];
    if (!current) {
    ...
    const movements = (await client.query(`SELECT COALESCE(SUM(CASE WHEN movement_type='cash_in' THEN amount ELSE -amount END),0) total FROM pos_shift_movements WHERE shift_id=$1`, [req.params.id])).rows[0];
