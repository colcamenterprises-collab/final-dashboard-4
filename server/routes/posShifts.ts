...
const testSource = "sbb_pos_test_sandbox";
...
// ---------------------------------------------------------------------------
// Manager Test Mode
// All state below lives in pos_test_* tables and is deliberately isolated from
// live shifts, ordering_orders, ordering_payments, sales reports and banking.
// ---------------------------------------------------------------------------

async function currentTestShift() {
  const result = await pool.query(`SELECT * FROM public.pos_test_shifts WHERE status='open' ORDER BY opened_at DESC LIMIT 1`);
  return result.rows[0] || null;
}
...
async function testCashSales(shiftId: string) {
  const result = await pool.query(
    `SELECT COALESCE(SUM(total),0) AS total
       FROM public.pos_test_orders
      WHERE shift_id=$1 AND payment_method='cash' AND status <> 'cancelled'`,
    [shiftId],
  );
  return numberValue(result.rows[0]?.total);
}

router.get("/test/access", staffDevice, managerOnly, async (req, res) => {
  const user = getPinSessionUser(req);
  res.json({ ok: true, source: testSource, data: { authorised: true, user } });
});

router.get("/test/current", staffDevice, managerOnly, async (_req, res) => {
  try {
    const shift = await currentTestShift();
    const movements = shift ? (await pool.query(`SELECT * FROM public.pos_test_shift_movements WHERE shift_id=$1 ORDER BY created_at DESC`, [shift.id])).rows : [];
    const history = (await pool.query(`SELECT * FROM public.pos_test_shifts ORDER BY opened_at DESC LIMIT 20`)).rows;
    const cashSales = shift ? await testCashSales(shift.id) : 0;
    res.json({ ok: true, source: testSource, test_mode: true, data: { shift, movements, history, cashSales } });
  } catch (error: any) {
    fail(res, error.message, 500);
  }
});

router.post("/test/open", staffDevice, managerOnly, async (req, res) => {
  try {
    ...
    const result = await pool.query(`INSERT INTO public.pos_test_shifts(staff_name,starting_float,opened_by) VALUES($1,$2,$3) RETURNING *`, [staffName, startingFloat, actor]);
    res.status(201).json({ ok: true, source: testSource, test_mode: true, data: result.rows[0] });
  } catch (error: any) {
...
router.post("/test/:id/movements", staffDevice, managerOnly, async (req, res) => {
  try {
    const movementType = req.body?.movement_type;
    const amount = numberValue(req.body?.amount);
    const reason = textValue(req.body?.reason);
    if (!["cash_in", "cash_out"].includes(movementType) || amount <= 0 || !reason) return fail(res, "Movement type, amount and reason are required");
    const open = await pool.query(`SELECT id FROM public.pos_test_shifts WHERE id=$1 AND status='open'`, [req.params.id]);
    if (!open.rowCount) return fail(res, "Test shift is not open", 409);
    const user = getPinSessionUser(req);
    const actor = user?.name || String(user?.id || "manager");
    const result = await pool.query(`INSERT INTO public.pos_test_shift_movements(shift_id,movement_type,amount,reason,created_by) VALUES($1,$2,$3,$4,$5) RETURNING *`, [req.params.id, movementType, amount, reason, actor]);
    res.status(201).json({ ok: true, source: testSource, test_mode: true, data: result.rows[0] });
  } catch (error: any) {
    fail(res, error.message, 500);
  }
...
router.post("/test/:id/close", staffDevice, managerOnly, async (req, res) => {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    ...
    if (!current) {
      await client.query("ROLLBACK");
      return fail(res, "Test shift is not open", 409);
    }
    const closingCash = numberValue(req.body?.closing_cash);
    const cashBanked = numberValue(req.body?.cash_banked);
    if (closingCash < 0 || cashBanked < 0) {
    ...
    await client.query("COMMIT");
    res.json({ ok: true, source: testSource, test_mode: true, data: { ...closed, cash_sales: cashSales } });
  } catch (error: any) {
    await client.query("ROLLBACK");
    fail(res, error.message, 500);
  } finally {
...
router.get("/test/orders/next-ticket", staffDevice, managerOnly, async (_req, res) => {
  try {
    const shift = await currentTestShift();
    if (!shift) return fail(res, "Open a test shift before taking test orders", 409);
    const result = await pool.query(`SELECT COUNT(*)::int + 1 AS next_number FROM public.pos_test_orders WHERE shift_id=$1`, [shift.id]);
    const next = Number(result.rows[0]?.next_number || 1);
    res.json({ ok: true, source: testSource, test_mode: true, data: { ticket_number: String(((next - 1) % 999) + 1).padStart(3, "0") } });
  } catch (error: any) {
    fail(res, error.message, 500);
  }
});

router.post("/test/orders", staffDevice, managerOnly, async (req, res) => {
  ...
  try {
    ...
    for (const line of input.items) {
      ...
      const modifierIds: string[] = Array.isArray(line.modifier_ids) ? [...new Set<string>(line.modifier_ids.filter((id: unknown): id is string => typeof id === "string"))] : [];
      const selectedModifiers = modifierIds.length ? (await client.query(
        `SELECT m.*,g.name_en AS modifier_group_name_en,g.group_type
           FROM public.ordering_item_modifiers m
           JOIN public.ordering_modifier_groups g ON g.id=m.modifier_group_id
          WHERE m.id=ANY($1::uuid[]) AND m.is_active AND g.is_active
            AND (g.menu_item_id=$2 OR EXISTS(SELECT 1 FROM public.ordering_modifier_group_items a WHERE a.modifier_group_id=g.id AND a.menu_item_id=$2))`,
        [modifierIds, item.id],
      )).rows : [];
      if (selectedModifiers.length !== modifierIds.length) throw new Error("Invalid option for this item");

      const modifiers = selectedModifiers.map((modifier: any) => ({
        id: modifier.id,
        name_en: modifier.name_en,
        modifier_name_en: modifier.name_en,
        name_th: modifier.name_th,
        price_delta: numberValue(modifier.price_delta),
        quantity,
      }));
      ...
      const receiptItem: any = {
        id: `${item.id}-${receiptItems.length}`,
        ...
        notes: textValue(line.notes, 240) || null,
      ...
      if (mode === "direct" && line.set_upgrade) {
        ...
        receiptItems.push({ id: `set-fries-${receiptItems.length}`, menu_item_id: fries.id, item_name_en: fries.name_en, item_name_th: fries.name_th, unit_price: 0, quantity, line_total: 0, notes: null, is_set_component: true, modifiers: [] });
        receiptItems.push({ id: `set-drink-${receiptItems.length}`, menu_item_id: drink.id, item_name_en: drink.name_en, item_name_th: drink.name_th, unit_price: 0, quantity, line_total: 0, notes: null, is_set_component: true, modifiers: [] });
      } else {
        receiptItems.push(receiptItem);
      }
      subtotal += lineTotal;
    ...
    if (requestedDiscount) {
      const discount = (await client.query(
        `SELECT code,name,discount_type,value FROM public.pos_discount_codes
    ...
    const payload = {
      items: receiptItems,
      dining_type: input.dining_type || "takeaway",
      order_notes: input.order_notes || null,
      channel: mode === "grab" ? "grab" : "pos_direct",
      pos_origin_channel: mode === "grab" ? "grab" : "pos_direct",
      discount_code: discountCode,
    };

    const created = (await client.query(
      `INSERT INTO public.pos_test_orders(shift_id,ticket_number,order_mode,payment_method,status,grab_order_number,customer_name,customer_mobile,subtotal,discount_amount,total,payload)
       VALUES($1,$2,$3,$4,'submitted',$5,$6,$7,$8,$9,$10,$11::jsonb) RETURNING *`,
      [shift.id, ticket, mode, input.payment_method, grabOrderNumber, mode === "grab" ? customerName : null, mode === "grab" ? customerMobile : null, subtotal, discountAmount, total, JSON.stringify(payload)],
    )).rows[0];
    await client.query("COMMIT");
    res.status(201).json({ ok: true, source: testSource, test_mode: true, data: { id: created.id, ticket_number: ticket, receipt_number: ticket, shift_id: shift.id, subtotal, discount_amount: discountAmount, total, created_at: created.created_at } });
  } catch (error: any) {
    await client.query("ROLLBACK");
...
router.get("/test/orders/:id/receipt", staffDevice, managerOnly, async (req, res) => {
  try {
    const row = (await pool.query(`SELECT * FROM public.pos_test_orders WHERE id=$1`, [req.params.id])).rows[0];
    if (!row) return fail(res, "Test receipt not found", 404);
    const payload = row.payload || {};
    res.json({ ok: true, source: testSource, test_mode: true, data: { ...row, ticket_number: row.ticket_number, items: payload.items || [] } });
  } catch (error: any) {
    fail(res, error.message, 500);
  }
});

router.post("/test/orders/:id/print-event", staffDevice, managerOnly, async (_req, res) => {
  res.json({ ok: true, source: testSource, test_mode: true });
});

router.get("/test/kitchen/orders", staffDevice, managerOnly, async (_req, res) => {
  try {
    const shift = await currentTestShift();
    if (!shift) return res.json({ ok: true, source: testSource, test_mode: true, data: [] });
    const rows = (await pool.query(`SELECT * FROM public.pos_test_orders WHERE shift_id=$1 AND status NOT IN ('completed','cancelled') ORDER BY created_at ASC`, [shift.id])).rows;
    ...
    res.json({ ok: true, source: testSource, test_mode: true, data });
  } catch (error: any) {
    fail(res, error.message, 500);
  }
});

router.patch("/test/orders/:id/status", staffDevice, managerOnly, async (req, res) => {
  try {
    const status = String(req.body?.status || "");
    if (!["accepted", "preparing", "ready", "completed", "cancelled"].includes(status)) return fail(res, "Invalid test order status");
    const updated = (await pool.query(`UPDATE public.pos_test_orders SET status=$2,updated_at=NOW() WHERE id=$1 RETURNING *`, [req.params.id, status])).rows[0];
    if (!updated) return fail(res, "Test order not found", 404);
    res.json({ ok: true, source: testSource, test_mode: true, data: updated });
  } catch (error: any) {
    fail(res, error.message, 500);
  }
});

router.get("/test/display/orders", staffDevice, managerOnly, async (_req, res) => {
  try {
    const shift = await currentTestShift();
    if (!shift) return res.json({ ok: true, source: testSource, test_mode: true, data: [] });
    const rows = (await pool.query(`SELECT id,ticket_number,created_at FROM public.pos_test_orders WHERE shift_id=$1 AND status='ready' ORDER BY created_at ASC`, [shift.id])).rows;
    res.json({ ok: true, source: testSource, test_mode: true, data: rows });
  } catch (error: any) {
    fail(res, error.message, 500);
  }
});

router.delete("/test/reset", staffDevice, managerOnly, async (_req, res) => {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    ...
    await client.query("COMMIT");
    res.json({ ok: true, source: testSource, test_mode: true, data: { reset: true } });
  } catch (error: any) {
    await client.query("ROLLBACK");
    fail(res, error.message, 500);
  } finally {
