...
router.get('/v2', async (req, res) => {
  ...
  const drinksTable = Array.from(drinkDirectBySku.values())
    .map((row) => {
  ...
  const burgersAndSets = usageRows
    .map((row) => {
      const rule = (row.sku && ruleBySku.get(String(row.sku))) || ruleByName.get(normalizeName(String(row.item_name || '')));
      if (!rule) return null;
    ...
    .filter(Boolean) as Array<{ itemName: string; soldCount: number; type: 'Single' | 'Double' | 'Set' }>;

  const sideOrders = usageRows
    .map((row) => {
      const rule = (row.sku && ruleBySku.get(String(row.sku))) || ruleByName.get(normalizeName(String(row.item_name || '')));
    ...
    .filter(Boolean) as Array<{ itemName: string; soldCount: number }>;

  const modifierTable: Array<{ modifierType: string; item: string; count: number }> = [];
