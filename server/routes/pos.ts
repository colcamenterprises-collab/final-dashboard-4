...
router.get("/menu/:menuItemId/modifiers", staffDevice, async (req,res) => {
  try { const result = await db().query(`SELECT m.*,g.name_en AS modifier_group_name_en,g.name_th AS modifier_group_name_th FROM ordering_item_modifiers m JOIN ordering_modifier_groups g ON g.id=m.modifier_group_id WHERE g.menu_item_id=$1 AND m.is_active ORDER BY g.sort_order,m.sort_order`,[req.params.menuItemId]); res.json({ok:true,source:"sbb_pos_core",data:result.rows}); }
  catch (e:any) { fail(res,e.message,500); }
});
router.post("/orders", staffDevice, async (req, res) => {
  ...
  try { await client.query("BEGIN"); const order=(await client.query(`INSERT INTO ordering_orders(channel,order_mode,dining_type,order_notes,status,payment_status,payment_method) VALUES($1,$2,$3,$4,'submitted','paid',$5) RETURNING *`,[mode === "grab" ? "grab" : "pos_direct",mode,input.dining_type||null,input.order_notes||null,input.payment_method])).rows[0];
    ...
    for(const line of input.items){ const item=(await client.query(`SELECT * FROM ordering_menu_items WHERE id=$1 AND is_active AND pos_enabled AND NOT is_sold_out`,[line.menu_item_id])).rows[0]; if(!item) throw new Error("POS item unavailable"); const qty=Math.max(1,Math.trunc(value(line.quantity)||1)); const unit=value(mode === "grab" ? item.grab_price : item.direct_price ?? item.price); if(!unit) throw new Error(`${item.name_en} has no ${mode} price`); if(mode === "direct" && item.set_upgrade_eligible && line.set_upgrade === undefined) throw new Error(`Set decision required for ${item.name_en}`); if(mode === "grab" && line.set_upgrade) throw new Error("Grab orders cannot use staff upsells");
      ...
      if(modifierIds.length){const result=await client.query(`SELECT m.*,g.name_en AS modifier_group_name_en FROM ordering_item_modifiers m JOIN ordering_modifier_groups g ON g.id=m.modifier_group_id WHERE m.id=ANY($1::uuid[]) AND g.menu_item_id=$2 AND m.is_active`,[modifierIds,item.id]);if(result.rows.length !== modifierIds.length) throw new Error("Invalid modifier for this item");for(const modifier of result.rows){const delta=value(modifier.price_delta)*qty;await client.query(`INSERT INTO ordering_order_item_modifiers(order_item_id,item_modifier_id,modifier_group_name_en,modifier_name_en,modifier_name_th,price_delta,quantity) VALUES($1,$2,$3,$4,$5,$6,$7)`,[parent.id,modifier.id,modifier.modifier_group_name_en,modifier.name_en,modifier.name_th,value(modifier.price_delta),qty]);await client.query(`UPDATE ordering_order_items SET line_total=line_total+$2 WHERE id=$1`,[parent.id,delta]);total+=delta;}}
      if(mode === "direct" && line.set_upgrade){ if(!line.set_drink_menu_item_id) throw new Error("Set drink selection is required"); const [friesResult,drinkResult,setting]=await Promise.all([client.query(`SELECT * FROM ordering_menu_items WHERE lower(name_en)=lower('French Fries') AND is_active AND pos_enabled LIMIT 1`),client.query(`SELECT * FROM ordering_menu_items WHERE id=$1 AND is_active AND pos_enabled`,[line.set_drink_menu_item_id]),client.query(`SELECT value FROM ordering_settings WHERE key='pos_set_upgrade_amount'`)]); const fries=friesResult.rows[0], drink=drinkResult.rows[0], upgrade=value(setting.rows[0]?.value||80); if(!fries||!drink) throw new Error("Set fries or drink is not configured"); await client.query(`UPDATE ordering_order_items SET line_total=line_total+$2 WHERE id=$1`,[parent.id,upgrade*qty]); await client.query(`INSERT INTO ordering_order_item_modifiers(order_item_id,modifier_group_name_en,modifier_name_en,price_delta,quantity) VALUES($1,'SET UPGRADE','Burger + French Fries + Drink',$2,$3)`,[parent.id,upgrade,qty]); for(const component of [fries,drink]) await client.query(`INSERT INTO ordering_order_items(order_id,menu_item_id,item_name_en,item_name_th,unit_price,quantity,line_total,sort_order,source_sku,price_mode,is_set_component,parent_order_item_id) VALUES($1,$2,$3,$4,0,$5,0,$6,$7,$8,true,$9)`,[order.id,component.id,component.name_en,component.name_th,qty,sort++,component.source_sku||null,mode,parent.id]); total+=upgrade*qty; }
    }
    await client.query(`UPDATE ordering_orders SET subtotal=$2,total=$2 WHERE id=$1`,[order.id,total]); await client.query(`INSERT INTO ordering_payments(order_id,method,status,amount) VALUES($1,$2,'confirmed',$3)`,[order.id,input.payment_method,total]); await client.query(`INSERT INTO pos_order_events(order_id,event_type,payload) VALUES($1,'order_created',$2)`,[order.id,JSON.stringify({ticket_number:ticket})]); await client.query("COMMIT"); res.status(201).json({ok:true,source:"sbb_pos_core",data:{id:order.id,ticket_number:ticket,total}});
