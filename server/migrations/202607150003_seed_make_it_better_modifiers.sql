-- POS modifier seed: one Make it Better group attached to each burger.
...
WHERE c.name_en IN ('Burgers','Chicken Burgers')
  AND NOT EXISTS (SELECT 1 FROM ordering_modifier_groups g WHERE g.menu_item_id=i.id AND g.name_en='Make it Better');
...
WHERE g.name_en='Make it Better'
  AND NOT EXISTS (SELECT 1 FROM ordering_item_modifiers m WHERE m.modifier_group_id=g.id AND m.name_en=v.name_en);
