...
async function main() {
  ...
  const result = await prisma.$transaction(async (tx) => {
    const revenueRows = await tx.$queryRawUnsafe<any[]>(
    ...
    const evidenceRows = await tx.$queryRawUnsafe<any[]>(
    ...
    // Keep the batch non-authoritative until every transaction insert and final
    // count update succeeds. The entire callback is a single database transaction,
    // so any error/interruption rolls back the batch and all rows together.
    const batchRows = await tx.$queryRawUnsafe<any[]>(
      `INSERT INTO financial_import_batches
       ...
       VALUES ($1,$2::uuid,$3,$4,$5::timestamptz,$6::timestamptz,$7::timestamptz,$8::timestamptz,$9,$10,0,$11,'importing',$12::jsonb)
    ...
    for (const row of parsed.accepted) {
      ...
      if (Number(insertCount) > 0) inserted += 1;
      else duplicates += 1;
    }

    await tx.$executeRawUnsafe(
      `UPDATE financial_import_batches
       SET accepted_count=$2, duplicate_count=$3, status='validated'
       WHERE id=$1::uuid`,
      batchId,
      inserted,
      duplicates,
    ...
    return { batchId, evidenceSourceId, revenueSourceId, inserted, duplicates };
  });
