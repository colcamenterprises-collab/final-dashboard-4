...
export function registerRoutes(app: express.Application): Server {
  ...
  // Send form as email with PDF attachment
  app.post("/api/daily-stock-sales/:id/send-email", async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      ...
      doc.on('end', async () => {
        try {
          ...
          const shiftDate = new Date(formData.shiftDate).toLocaleDateString();
          const emailHTML = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px;">
              <h2>Daily Shift Report - Form ${formData.id}</h2>
              <p><strong>Date:</strong> ${shiftDate}</p>
              ...
              <h3>Sales Summary</h3>
              <ul>
                <li>Total Sales: THB ${parseFloat(formData.totalSales || '0').toFixed(2)}</li>
                <li>Cash Sales: THB ${parseFloat(formData.cashSales || '0').toFixed(2)}</li>
                <li>Grab Sales: THB ${parseFloat(formData.grabSales || '0').toFixed(2)}</li>
                <li>QR Scan Sales: THB ${parseFloat(formData.qrScanSales || '0').toFixed(2)}</li>
              </ul>
              
              <h3>Cash Management</h3>
              <ul>
                <li>Starting Cash: THB ${parseFloat(formData.startingCash || '0').toFixed(2)}</li>
                <li>Ending Cash: THB ${parseFloat(formData.endingCash || '0').toFixed(2)}</li>
                <li>Banked Amount: THB ${parseFloat(formData.bankedAmount || '0').toFixed(2)}</li>
              </ul>
              
              <p>Please find the complete daily shift report attached as a PDF.</p>
              
              <p style="color: #666; font-size: 12px;">Generated by Smash Brothers Burgers Management System</p>
            </div>
          `;
          ...
          const emailSent = await sendEmailWithAttachment(
            email,
            `Daily Shift Report - Form ${formData.id} - ${shiftDate}`,
            emailHTML,
            [{
              filename: `form-${numericId}-daily-shift-report.pdf`,
              content: base64PDF,
              encoding: 'base64'
            }]
          );
          ...
          if (emailSent) {
            res.json({ 
              success: true, 
              message: `Form ${numericId} sent successfully to ${email}` 
            });
          } else {
  ...
  app.post("/api/send-form-summary", async (req: Request, res: Response) => {
    try {
      const { submissionId, emailTo, formData } = req.body;
      console.log("📧 Send form summary request:", { submissionId, emailTo });
      
      if (!submissionId || !emailTo) {
        return res.status(400).json({ error: "Missing submissionId or emailTo" });
      }
        ...
        try {
          ...
          const emailHTML = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px;">
              ...
              <p><strong>Form ID:</strong> ${submissionId}</p>
              <p><strong>Date:</strong> ${shiftDate}</p>
              <p><strong>Shift:</strong> ${formData.shiftType}</p>
              <p><strong>Completed by:</strong> ${formData.completedBy}</p>
              ...
              <ul>
                <li>Total Sales: THB ${parseFloat(formData.totalSales || '0').toFixed(2)}</li>
                <li>Grab Sales: THB ${parseFloat(formData.grabSales || '0').toFixed(2)}</li>
                <li>FoodPanda Sales: THB ${parseFloat(formData.foodPandaSales || '0').toFixed(2)}</li>
                <li>Aroi Dee Sales: THB ${parseFloat(formData.aroiDeeSales || '0').toFixed(2)}</li>
                <li>QR Scan Sales: THB ${parseFloat(formData.qrScanSales || '0').toFixed(2)}</li>
                <li>Cash Sales: THB ${parseFloat(formData.cashSales || '0').toFixed(2)}</li>
              </ul>
              
              <h3>Core Stock Tracking</h3>
              <ul>
                <li>Meat: ${formData.meat || 0} grams</li>
                <li>Rolls: ${formData.rolls || 0} units</li>
                <li>Drinks: ${formData.drinks || 0} units</li>
              </ul>
              ...
              <h3>Expenses</h3>
              <ul>
                <li>Salary/Wages: THB ${parseFloat(formData.salaryWages || '0').toFixed(2)}</li>
                <li>Shopping: THB ${parseFloat(formData.shopping || '0').toFixed(2)}</li>
              </ul>
              
              <p>Complete daily shift report attached as PDF.</p>
              <p style="color: #666; font-size: 12px;">Generated by Smash Brothers Burgers Management System</p>
            </div>
          `;
          ...
          const emailSent = await sendEmailWithAttachment(
            ...
            emailHTML,
            [{
              filename: `daily-report-${submissionId}-${new Date().toISOString().split('T')[0]}.pdf`,
              content: base64PDF,
              encoding: 'base64'
            }]
          );
          ...
          if (emailSent) {
            console.log(`✅ Form summary email sent to ${emailTo}`);
            res.json({ success: true, message: `Email sent successfully to ${emailTo}` });
          } else {
            console.log(`❌ Failed to send email to ${emailTo}`);
            res.status(500).json({ error: "Failed to send email" });
          }
        } catch (error) {
          console.error('Error sending form summary email:', error);
          res.status(500).json({ error: 'Failed to send email' });
        }
      });
      ...
      doc.fontSize(16).font('Helvetica')
         .text('Daily Sales & Stock Report', 40, 70)
         .fontSize(12)
         .text(`Form ID: ${submissionId}`, 40, 95)
         .text(`Date: ${new Date(formData.shiftDate).toLocaleDateString()}`, 40, 110)
         .text(`Shift: ${formData.shiftType}`, 40, 125)
         .text(`Completed by: ${formData.completedBy}`, 40, 140);
      ...
      const grabSales = parseFloat(formData.grabSales || '0');
      const foodPandaSales = parseFloat(formData.foodPandaSales || '0');
      const aroiDeeSales = parseFloat(formData.aroiDeeSales || '0');
      const qrScanSales = parseFloat(formData.qrScanSales || '0');
      const cashSales = parseFloat(formData.cashSales || '0');
      const totalSales = parseFloat(formData.totalSales || '0');
      ...
      doc.fontSize(11).font('Helvetica')
         .text(`Grab Sales: THB ${grabSales.toFixed(2)}`, 40, y)
         .text(`FoodPanda Sales: THB ${foodPandaSales.toFixed(2)}`, 200, y)
         .text(`Aroi Dee Sales: THB ${aroiDeeSales.toFixed(2)}`, 350, y);
      y += 20;
      
      doc.text(`QR Scan Sales: THB ${qrScanSales.toFixed(2)}`, 40, y)
         .text(`Cash Sales: THB ${cashSales.toFixed(2)}`, 200, y)
         .text(`TOTAL SALES: THB ${totalSales.toFixed(2)}`, 350, y);
      y += 40;

      // Core Stock Tracking
      doc.fontSize(14).font('Helvetica-Bold')
         .text('CORE STOCK TRACKING', 40, y);
      y += 25;
      
      const meat = parseFloat(formData.meat || '0');
      const rolls = parseFloat(formData.rolls || '0');
      const drinks = parseFloat(formData.drinks || '0');
      
      doc.fontSize(11).font('Helvetica')
         .text(`Meat: ${meat} grams`, 40, y)
         .text(`Rolls: ${rolls} units`, 200, y)
         .text(`Drinks: ${drinks} units`, 350, y);
      y += 40;
      ...
      const salaryWages = parseFloat(formData.salaryWages || '0');
      const shoppingSales = parseFloat(formData.shopping || '0');
      
      doc.fontSize(11).font('Helvetica')
         .text(`Salary/Wages: THB ${salaryWages.toFixed(2)}`, 40, y)
         .text(`Shopping: THB ${shoppingSales.toFixed(2)}`, 200, y);
      y += 30;

      // Footer
      doc.fontSize(8).font('Helvetica')
         .text(`Generated: ${new Date().toLocaleString()}`, 40, doc.page.height - 50)
         .text('Smash Brothers Burgers - Daily Sales & Stock Report', 40, doc.page.height - 35);

      doc.end();
    } catch (error) {
      console.error('Error in send-form-summary:', error);
      res.status(500).json({ error: 'Failed to send form summary' });
    }
  });
