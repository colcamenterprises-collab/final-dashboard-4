...
class LoyverseAPI {
  ...
  async syncTodaysReceipts(): Promise<number> {
    try {
      ...
      // Use the working importReceiptsV2 function to sync receipts into lv_receipt/lv_line_item/lv_modifier tables
      const { importReceiptsV2 } = await import('./services/loyverseImportV2');
      const utcStart = this.convertBangkokToUTC(shiftData.shiftPeriod.start);
      const utcEnd = this.convertBangkokToUTC(shiftData.shiftPeriod.end);
      
      const result = await importReceiptsV2(utcStart, utcEnd);
      console.log(`✅ Synced ${result.upserted} receipts for last completed shift (${shiftData.totalSales} baht)`);
  ...
  // DEPRECATED: This method had SQL syntax errors due to schema mismatch
  // Use loyverseImportV2.importReceiptsV2() instead for receipt syncing
  // Kept for reference only - DO NOT USE
  /*
  private async storeReceiptData(receipt: LoyverseReceipt): Promise<void> {
    // This method is deprecated - use importReceiptsV2 from ./services/loyverseImportV2 instead
    throw new Error('storeReceiptData is deprecated - use importReceiptsV2 instead');
  }
  */
}
