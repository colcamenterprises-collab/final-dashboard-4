...
- **Staff Operations Module (Phase 2)**: Comprehensive multi-business/multi-location staff management with 16 database tables, 9 enums, 50+ API endpoints, and 7 UI pages for managing staff, rosters, cleaning tasks, and attendance. Fully configurable with `businessLocationId` defaulting to 1.
...
- **Station Requirements System**: `shift_template_station_requirements` table stores per-template station slot config (workAreaId, requiredCount, priority). Auto-generate roster uses these to fill each slot by work area before falling back to maxStaff mode. UI: "Stations" button per shift template row in Settings → Shift Templates opens inline panel with add/delete for slots. CRUD at `/api/operations/staff/templates/:id/station-requirements`.
- **Staff Unavailability System**: `staff_unavailability` table stores date-range blocks (startDate, endDate, reasonType enum, notes, isApproved). Auto-generate roster excludes blocked staff from eligible candidates. UI: expanded row in Staff Management shows "Date-Specific Unavailability" section with add/approve/delete. CRUD at `/api/operations/staff/unavailability`. Reason types: unavailable, annual_leave, sick, preferred_off, other.
- **Staggered Breaks**: `generateBreaksForRoster` now staggers main and short breaks by 15 mins per staff index so no two staff are on break simultaneously. Breaks are auto-generated for every roster during auto-generate. Break times shown in assignment cards in Weekly Roster Planner.
- **Guided Roster Setup Banner**: `WeeklyRosterPlanner.tsx` shows a dismissable amber banner when setup is incomplete (missing templates, work areas, or active staff), guiding managers through the pre-requisite steps before auto-generating.
...
- **POS Truth Layer Hard Lock**: `lv_line_item` and `lv_modifier` are permanently locked as the ONLY POS truth sources. Protection layers: (1) DB CHECK constraint `qty_must_be_one` on lv_modifier.qty — rejects any insert where qty ≠ 1. (2) DB triggers `no_update_lv_line_item` and `no_update_lv_modifier` call `prevent_lv_update()` which raises `POS_TRUTH_LAYER_VIOLATION: UPDATE not allowed on <table>` — no UPDATE is possible on these tables by any path. (3) Import service (`loyverseImportV2.ts`) uses `ON CONFLICT DO NOTHING` for both tables — rows are immutable after first insert; DO UPDATE is only allowed on lv_receipt. (4) Analysis V3 routes (`server/routes/analysisV3.ts`) are SELECT-only, guarded by `assertPosTruthLayer()` and `assertItemSalesIntegrity()` / `assertModifierIntegrity()`. (5) Both endpoints carry mandatory comment blocks declaring POS TRUTH LAYER. (6) Checksums returned in every response: `total_rows`, `total_qty_sold`/`total_items_sold`, `total_receipts`, `source_tables`. (7) `status: "POS_DATA_INCOMPLETE"` returned if receipts exist but zero item/modifier rows returned — never faked. Multi-qty modifier rule: one row per selection per unit (Double Set x2 = 2 modifier rows, each qty=1).
