...
- **Theft-Control Issue Register**: `issue_register` table (auto-CREATE IF NOT EXISTS). CRUD API at `/api/issue-register/`. Auto-detect logic (`POST /api/issue-register/auto-create?date`) reads existing reconciliation signals from `getUsageReconciliation()` and `receipt_truth_line` — creates deduped issues for BUNS_VARIANCE, MEAT_VARIANCE, DRINK_VARIANCE, INVALID_STOCK_INPUT, POS_FORM_MISMATCH, MISSING_USAGE_BUILD. Status lifecycle: OPEN→IN_PROGRESS→RESOLVED→CLOSED. Deduped on `(shift_date, issue_type, source_ref)`. UI at `/operations/issue-register` with filters, expandable detail, resolution notes, manual create form. Summary widget embedded in SalesShiftAnalysis page for selected date.
