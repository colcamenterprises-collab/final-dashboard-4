...
const COSTING_UNAVAILABLE_MIGRATION = path.join(ROOT, 'server/migrations/202608080003_historical_costing_unavailable.sql');
...
function parseCsv(text) {
  ...
  return lines.filter(Boolean).map((line, index) => {
    ...
    return {
      ...
      discounts: parseNumber(cells[7]), netSales: parseNumber(cells[8]), taxes: parseNumber(cells[12]),
    };
  });
}
...
if (!fs.existsSync(COSTING_UNAVAILABLE_MIGRATION)) fail(`Historical costing cleanup migration missing: ${COSTING_UNAVAILABLE_MIGRATION}`);
...
const checks = {
  ...
  refunds: sum(rows, 'refunds'), discounts: sum(rows, 'discounts'), netSales: sum(rows, 'netSales'), taxes: sum(rows, 'taxes'),
};
...
console.log('CSV factual sales reconciliation: PASS');
console.log('Historical COGS / gross profit / margin: intentionally NOT imported');
...
try {
  ...
  await client.query(fs.readFileSync(COSTING_UNAVAILABLE_MIGRATION, 'utf8'));
  for (const row of rows) {
    await client.query(
      `INSERT INTO historical_item_sales (source,source_file,source_file_sha256,period_start,period_end,item_name,sku,category,items_sold,gross_sales,items_refunded,refunds,discounts,net_sales,cost_of_goods,gross_profit,margin_pct,taxes)
       ...
       ON CONFLICT (source,sku,period_start,period_end) DO UPDATE SET source_file=EXCLUDED.source_file,source_file_sha256=EXCLUDED.source_file_sha256,item_name=EXCLUDED.item_name,category=EXCLUDED.category,items_sold=EXCLUDED.items_sold,gross_sales=EXCLUDED.gross_sales,items_refunded=EXCLUDED.items_refunded,refunds=EXCLUDED.refunds,discounts=EXCLUDED.discounts,net_sales=EXCLUDED.net_sales,cost_of_goods=NULL,gross_profit=NULL,margin_pct=NULL,taxes=EXCLUDED.taxes,imported_at=NOW()`,
      [SOURCE,SOURCE_FILE,ORIGINAL_SOURCE_SHA,PERIOD_START,PERIOD_END,row.itemName,row.sku,row.category,row.itemsSold,row.grossSales,row.itemsRefunded,row.refunds,row.discounts,row.netSales,row.taxes],
    );
  }
  const result = await client.query(
    `SELECT COUNT(*)::int rows,COALESCE(SUM(items_sold),0)::numeric items_sold,COALESCE(SUM(gross_sales),0)::numeric gross_sales,COALESCE(SUM(items_refunded),0)::numeric items_refunded,COALESCE(SUM(refunds),0)::numeric refunds,COALESCE(SUM(discounts),0)::numeric discounts,COALESCE(SUM(net_sales),0)::numeric net_sales,COALESCE(SUM(taxes),0)::numeric taxes,COUNT(cost_of_goods)::int cogs_rows,COUNT(gross_profit)::int profit_rows,COUNT(margin_pct)::int margin_rows FROM historical_item_sales WHERE source=$1 AND source_file_sha256=$2`,
    [SOURCE, ORIGINAL_SOURCE_SHA],
  );
  const db = result.rows[0];
  const dbChecks = { rows:Number(db.rows),itemsSold:Number(db.items_sold),grossSales:Number(db.gross_sales),itemsRefunded:Number(db.items_refunded),refunds:Number(db.refunds),discounts:Number(db.discounts),netSales:Number(db.net_sales),taxes:Number(db.taxes) };
  for (const [key, expected] of Object.entries(EXPECTED)) {
    if (Math.abs(dbChecks[key] - expected) > 0.001) fail(`Database ${key} reconciliation failed: expected ${expected}; received ${dbChecks[key]}`);
  }
  if (Number(db.cogs_rows) !== 0 || Number(db.profit_rows) !== 0 || Number(db.margin_rows) !== 0) fail('Historical costing fields must all be NULL');
  ...
  console.log('COGS / Gross profit / Margin: UNAVAILABLE (NULL by policy)');
