...
if (source.includes("Product Builder</p>")) {
  console.log("Product Builder shell already applied.");
  process.exit(0);
}

const start = source.indexOf('  return <div className="fixed inset-0 z-50');
const end = source.lastIndexOf('\n}');
if (start < 0 || end < 0 || end <= start) throw new Error("Could not locate MenuItemEditor render block");

const render = String.raw`  const grabPrice = String((draft as any).grabPrice ?? (item as any).grabPrice ?? draft.price ?? "0");
  ...
  return <div className="fixed inset-0 z-50 bg-black/45 p-0 sm:p-3" onClick={onClose}>
    <div className="mx-auto flex h-full w-full max-w-[1500px] flex-col overflow-hidden bg-slate-50 shadow-2xl sm:rounded-2xl" onClick={(event) => event.stopPropagation()}>
      ...
      <footer className="border-t bg-white px-4 py-3 sm:px-6"><div className="mx-auto flex max-w-[1500px] flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-between"><div>{saveBlockedReason && <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800">{saveBlockedReason}</p>}{save.isError && <p className="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700">{(save.error as Error)?.message || "Could not save product"}</p>}</div><button disabled={Boolean(saveBlockedReason) || save.isPending} onClick={() => save.mutate()} className="rounded-xl bg-black px-8 py-3 text-sm font-bold text-white disabled:opacity-40">{save.isPending ? "Saving product…" : "Save product"}</button></div></footer>
    </div>
  </div>;`;

source = source.slice(0, start) + render + source.slice(end);
fs.writeFileSync(file, source);
console.log("Product Builder shell applied successfully.");
