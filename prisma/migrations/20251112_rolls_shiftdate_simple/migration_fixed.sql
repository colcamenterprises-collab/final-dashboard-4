...
BEGIN
  ...
  ELSIF table_exists('expenses') THEN
    -- Count bun/roll purchases from expenses
    SELECT COALESCE(COUNT(*),0)::int * 48 INTO v_purchased
    FROM expenses
    WHERE (lower(item) LIKE '%bun%' OR lower(item) LIKE '%roll%')
      AND shift_date = p_date;
  END IF;

  -- Burgers Sold: from analytics_shift_item (already computed)
...
-- 4) Trigger function for daily_stock_sales
CREATE OR REPLACE FUNCTION _rl_cb_dss()
...
-- 5) Trigger function for expenses
CREATE OR REPLACE FUNCTION _rl_cb_exp()
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
  d date;
BEGIN
