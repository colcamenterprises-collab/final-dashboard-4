// Builds a ShiftSnapshot from EXISTING DB receipts (no external pulls),
// maps payments, stores items/modifiers, and compares to DailySales (buns/meat/drinks).
import { PrismaClient } from '@prisma/client';
const prisma = new PrismaClient();

// ---- Config ----
const MEAT_PER_PATTY_GRAM = 90;   // your standard
const BUNS_PER_BURGER = 1;        // 1 bun set per burger item

function bkkWindowToUTC(dateStrYYYYMMDD) {
  // 18:00 BKK -> 11:00 UTC same day ; 03:00 BKK -> 20:00 UTC same day
  const [y, m, d] = dateStrYYYYMMDD.split('-').map(Number);
  const startUTC = new Date(Date.UTC(y, m - 1, d, 11, 0, 0)); // 18:00 BKK
  const endUTC   = new Date(Date.UTC(y, m - 1, d, 20, 0, 0)); // 03:00 BKK (next day local)
  return { startUTC, endUTC };
}

// Map provider-specific payment type -> canonical channel
...
async function runForDate(dateStr, opts = {}) {
  ...
  for (const p of payments) {
    ...
    await prisma.paymentBreakdown.upsert({
      ...
      create: {
        ...
        channel,
        count: p.count,
        totalSatang: BigInt(p.total_satang)
      },
      update: {
        count: p.count,
        totalSatang: BigInt(p.total_satang)
  ...
  // Quick Jussi comparison (heuristic until recipe map is ready)
  ...
  for (const it of itemsForCalc) {
    ...
    if (nm.includes('set') || nm.includes('meal')) {
      drinksSold += it.qty; // 1 drink per set (adjust later with recipe map)
    }
  }
  ...
  // Staff-reported from DailySales/DailyStock
  ...
  if (staffBuns == null || staffMeatGram == null || staffDrinks == null) {
    state = 'MISSING_DATA';
    notes.push('Missing staff stock counts for one or more key items.');
  }
  ...
  await prisma.shiftSnapshot.update({
    ...
    data: {
      ...
      reconcileNotes: notes.length ? notes.join(' ') : null
    }
  });
...
const dateArg = process.argv[2]; // e.g., 2025-08-09
if (!dateArg) {
