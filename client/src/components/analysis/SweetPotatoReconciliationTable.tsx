 ...
 * SWEET_POTATO_USAGE_SKUS: 10022 ONLY. 130g per serving.
 */

import { useState } from 'react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';

type UsageBreakdownRow = { sku: string; item_name: string; servings: number; grams: number };
...
type SweetPotatoResponse = {
  ...
  grams_per_serving?: number; total_servings?: number;
  usage_breakdown?: UsageBreakdownRow[]; stock_source?: { start: string; end: string };
  data: SweetPotatoData | null; error?: string;
};
...
function ReviewModal({ date, variance, existing, onClose }: { date: string; variance: number; existing: SweetPotatoData['latest_review']; onClose: () => void }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['/api/analysis/sweet-potato-reconciliation', date] }); onClose(); },
  });
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm mx-4 p-5">
        <h3 className="text-sm font-semibold text-slate-900 mb-1">Review Sweet Potato Variance</h3>
...
export default function SweetPotatoReconciliationTable({ date }: { date: string }) {
  ...
  const { data: resp, isLoading, isError, error } = useQuery<SweetPotatoResponse>({
    ...
    queryFn: () => fetch(`/api/analysis/sweet-potato-reconciliation?date=${date}`).then((r) => r.json()),
  ...
  if (isError || !resp?.ok) {
    return <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-xs text-red-700">Failed to load Sweet Potato data: {(error as Error)?.message ?? resp?.error ?? 'Unknown error'}</div>;
  }
  if (resp.status === 'incomplete_component_data' || resp.data === null) {
    return <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800"><span className="font-semibold">incomplete_component_data</span> — missing: {resp.missing?.join(', ') ?? 'unknown'}. Sweet potato end-of-shift count not yet recorded.</div>;
  }
      ...
      <div className="rounded-lg border border-slate-200 bg-white overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
          <div className="flex items-center gap-3">
            <span className="text-sm font-semibold text-slate-800">Sweet Potato Fries Stock Reconciliation</span>
            <span className="text-[11px] text-slate-400 border border-slate-200 rounded px-1.5 py-0.5">SKU 10022 · 130g / serving</span>
          </div>
