/**
 * SweetPotatoReconciliationTable — Sweet Potato Fries Stock Reconciliation
 *
 * Single-row reconciliation: Item | Start | Purchased | Used | End | Expected | Variance | Review
 * Route: GET /api/analysis/sweet-potato-reconciliation?date=YYYY-MM-DD
 *
 * SWEET_POTATO_USAGE_SKUS: 10022 ONLY — no set meals.
 * 130g per serving.
 *
 * Isolated — does NOT touch French Fries, Drinks, Burgers, Buns, Meat, or Side Orders.
 */
...
type SweetPotatoResponse = {
  ...
  stock_source?: { start: string; end: string };
  data: SweetPotatoData | null;
  error?: string;
};
...
function Th({ children, right }: { children: React.ReactNode; right?: boolean }) {
  return (
    <th
      className={`px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide bg-gray-50 border-b border-gray-200 whitespace-nowrap ${
    ...
    <div className="border-t border-gray-100 bg-gray-50">
      <div className="px-4 py-2">
        <p className="text-xs font-semibold text-gray-600 mb-2">
          Usage Breakdown — {gramsPerServing}g per serving
        </p>
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr>
              ...
              <th className="text-right px-2 py-1 text-gray-500 font-medium">Servings</th>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={`${r.sku}-${i}`}>
                ...
                <td className="px-2 py-1 text-right tabular-nums text-gray-700">{r.servings}</td>
                <td className="px-2 py-1 text-right tabular-nums text-gray-700">
                  {r.grams.toLocaleString()}g
                </td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="border-t-2 border-gray-300 bg-gray-100">
              <td colSpan={2} className="px-2 py-1 text-xs font-semibold text-gray-700 text-right">
                Total
              </td>
              ...
              <td className="px-2 py-1 text-right text-xs font-semibold tabular-nums text-gray-900">
                {(totalServings * gramsPerServing).toLocaleString()}g
              </td>
            </tr>
          </tfoot>
        </table>
...
function ReviewModal({
  ...
  date: string;
  variance: number;
  existing: SweetPotatoData['latest_review'];
  onClose: () => void;
}) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const resp = await fetch('/api/analysis/sweet-potato-reconciliation/reviews', {
        method: 'POST',
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/analysis/sweet-potato-reconciliation', date] });
      onClose();
    },
  });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm mx-4 p-5">
        <h3 className="text-sm font-semibold text-gray-900 mb-1">Review Sweet Potato Variance</h3>
        ...
        <textarea
          ...
          placeholder="Explain the variance (e.g. spillage, miscounted, delivery short…)"
          value={note}
...
export default function SweetPotatoReconciliationTable({ date }: { date: string }) {
  const [showReview, setShowReview] = useState(false);
  const [showBreakdown, setShowBreakdown] = useState(false);

  const { data: resp, isLoading, isError, error } = useQuery<SweetPotatoResponse>({
    queryKey: ['/api/analysis/sweet-potato-reconciliation', date],
    queryFn: () =>
      fetch(`/api/analysis/sweet-potato-reconciliation?date=${date}`).then((r) => r.json()),
    enabled: !!date,
    staleTime: 60_000,
  });
  ...
  if (isLoading) {
    return (
      <div className="rounded-lg border border-gray-200 bg-white p-4 text-xs text-gray-500 animate-pulse">
        Loading Sweet Potato Reconciliation…
      </div>
    );
  }
  ...
  if (isError || !resp?.ok) {
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-xs text-red-700">
        Failed to load Sweet Potato data:{' '}
  ...
  if (resp.status === 'incomplete_component_data' || resp.data === null) {
    return (
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
        ...
        {resp.missing?.join(', ') ?? 'unknown'}. Sweet potato end-of-shift count not yet recorded for
        this date.
      </div>
    );
  }
      ...
      <div className="rounded-lg border border-gray-200 bg-white overflow-hidden">
        ...
        <div className="px-4 py-2 border-t border-gray-100 flex items-center justify-between gap-2 flex-wrap">
          <span className="text-xs text-gray-400">
            ...
            &nbsp;·&nbsp; Source: {resp.stock_source?.start ?? '—'}
            {row.latest_review && (
              <span className="ml-3 text-green-600">✓ "{row.latest_review.owner_note}"</span>
            )}
          </span>
          {resp.usage_breakdown && resp.usage_breakdown.length > 0 && (
            ...
            >
              {showBreakdown
                ? 'Hide breakdown'
                : `Show usage breakdown (${resp.total_servings} servings)`}
            </button>
          )}
        </div>
        ...
        {showBreakdown && resp.usage_breakdown && (
          <UsageBreakdown
            rows={resp.usage_breakdown}
            totalServings={resp.total_servings ?? 0}
            gramsPerServing={resp.grams_per_serving ?? 130}
          />
        )}
      </div>
    </>
