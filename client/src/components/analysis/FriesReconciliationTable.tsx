/**
 * FriesReconciliationTable — Core Stock V4, French Fries (PATCH 1)
 *
 * Single-row reconciliation: Item | Start | Purchased | Used | End | Expected | Variance | Review
 * Route: GET /api/analysis/fries-reconciliation?date=YYYY-MM-DD
 *
 * FRIES_USAGE_SKUS: 10018, 10010, 10045, 10030, 10035
 * Sweet Potato Fries (10022) excluded.
 * 130g per serving. Purchased = 0 (Patch 1).
 *
 * Isolated — does NOT touch Drinks, Burgers & Sets, Buns, Meat, or Side Orders.
 */
        ...
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr>
              ...
              <th className="text-left px-2 py-1 text-gray-500 font-medium">Source</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
              <tr
                key={`${r.sku}-${i}`}
                className={r.source === 'set_meal' ? 'bg-blue-50' : ''}
              >
...
function ReviewModal({
  ...
  date: string;
  variance: number;
  existing: FriesData['latest_review'];
  onClose: () => void;
}) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/analysis/fries-reconciliation', date] });
      onClose();
    },
  });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm mx-4 p-5">
        <h3 className="text-sm font-semibold text-gray-900 mb-1">Review French Fries Variance</h3>
        ...
        <textarea
          ...
          placeholder="Explain the variance (e.g. spillage, miscounted bags, delivery short…)"
          value={note}
...
export default function FriesReconciliationTable({ date }: { date: string }) {
  ...
  const { data: resp, isLoading, isError, error } = useQuery<FriesResponse>({
    ...
    queryFn: () =>
      fetch(`/api/analysis/fries-reconciliation?date=${date}`).then((r) => r.json()),
    enabled: !!date,
    staleTime: 60_000,
  });
  ...
  if (isLoading) {
    return (
      <div className="rounded-lg border border-gray-200 bg-white p-4 text-xs text-gray-500 animate-pulse">
        Loading French Fries Reconciliation…
      </div>
    );
  }
  ...
  if (isError || !resp?.ok) {
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-xs text-red-700">
        Failed to load French Fries data:{' '}
  ...
  if (resp.status === 'incomplete_component_data' || resp.data === null) {
    return (
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
        ...
        {resp.missing?.join(', ') ?? 'unknown'}. Fries stock (friesEnd) not yet recorded for this
        date.
      </div>
    );
  }
