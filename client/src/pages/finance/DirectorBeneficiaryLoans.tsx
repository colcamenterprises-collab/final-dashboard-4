      ...
      <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700">
        <table className="w-full min-w-[720px] text-xs">
          ...
          <tbody>
            ...
            {isLoading && <tr><td colSpan={isOwner ? 5 : 4} className="px-3 py-8 text-center text-slate-400">Loading loans...</td></tr>}
            {!isLoading && loans.length === 0 && !adding && <tr><td colSpan={isOwner ? 5 : 4} className="px-3 py-8 text-center text-slate-400">No director / beneficiary loans recorded.</td></tr>}
            {loans.map((loan) => {
              ...
              return <tr key={loan.id} className="border-t border-slate-100 dark:border-slate-800">
                ...
                {isOwner && <td className="px-2 py-1"><div className="flex justify-end gap-1">{editing ? <><Button size="icon" variant="ghost" className="h-7 w-7" disabled={!valid || save.isPending} onClick={() => save.mutate({ id: loan.id, values: draft })}><Save className="h-3.5 w-3.5" /></Button><Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setEditingId(null); setDraft(emptyDraft); }}><X className="h-3.5 w-3.5" /></Button></> : <><Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => beginEdit(loan)}><Pencil className="h-3.5 w-3.5" /></Button><Button size="icon" variant="ghost" className="h-7 w-7 text-red-600" onClick={() => { if (window.confirm("Delete this loan record?")) remove.mutate(loan.id); }}><Trash2 className="h-3.5 w-3.5" /></Button></>}</div></td>}
              </tr>;
            })}
          </tbody>
          <tfoot><tr className="border-t-2 border-slate-200 bg-slate-50 font-semibold dark:border-slate-700 dark:bg-slate-800"><td className="px-3 py-2 text-right font-mono">{money(loans.reduce((sum, loan) => sum + Number(loan.amount || 0), 0))}</td><td colSpan={2} className="px-3 py-2 text-slate-500">Total</td><td className="px-3 py-2 text-right font-mono">{money(loans.reduce((sum, loan) => sum + Number(loan.balance || 0), 0))}</td>{isOwner && <td />}</tr></tfoot>
        </table>
      </div>
    </section>
  );
