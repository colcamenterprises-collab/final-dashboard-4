...
import { z } from 'zod';
import { Plus, Pencil, Trash2, Save, Clock, MapPin, Layout, ClipboardList } from 'lucide-react';
...
type Settings = {
  id: number;
  defaultMaxStaffPerShift: number;
  breakMainMinutes: number;
  breakShortMinutes: number;
  breakShortCount: number;
  allowPrepShift: boolean;
...
type Tab = 'break-rules' | 'operating-hours' | 'work-areas' | 'templates' | 'daily-cleaning';
const TABS: { id: Tab; label: string; icon: React.ElementType }[] = [
  ...
  { id: 'daily-cleaning', label: 'Daily Cleaning', icon: ClipboardList },
];
...
export default function StaffOpsSettings() {
  ...
  const { data: settings, isLoading: loadingSettings } = useQuery<Settings>({
    queryKey: ['/api/operations/staff/settings'],
  });
  const { data: opHours = [] } = useQuery<OperatingHours[]>({
    queryKey: ['/api/operations/staff/operating-hours'],
  });
  const { data: workAreas = [], isLoading: loadingAreas } = useQuery<WorkArea[]>({
  ...
  const { data: cleaningTemplates = [], isLoading: loadingCleaningTemplates } = useQuery<
    CleaningTemplate[]
  >({ queryKey: ['/api/operations/staff/cleaning/templates'] });
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-500 mt-0.5">
          Configure break rules, shift templates, work areas and operating hours.
        </p>
      </div>
      ...
      {tab === 'break-rules' && (
        <BreakRulesPanel settings={settings} loading={loadingSettings} qc={qc} toast={toast} />
      )}
      ...
      {tab === 'daily-cleaning' && (
        <DailyCleaningTemplatesPanel
          ...
          loading={loadingCleaningTemplates}
          qc={qc}
          toast={toast}
        />
      )}
...
function BreakRulesPanel({
  ...
  const saveMut = useMutation({
    mutationFn: (data: z.infer<typeof settingsSchema>) =>
      sapi('PATCH', '/api/operations/staff/settings', data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/operations/staff/settings'] });
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <Field label="Max staff per shift">
          <input type="number" {...form.register('defaultMaxStaffPerShift')} className={inputCls} />
        </Field>
        <Field label="Main break (minutes)">
          <input type="number" {...form.register('breakMainMinutes')} className={inputCls} />
        </Field>
        <Field label="Short break (minutes)">
          <input type="number" {...form.register('breakShortMinutes')} className={inputCls} />
        </Field>
        <Field label="Short breaks per shift">
          <input type="number" {...form.register('breakShortCount')} className={inputCls} />
        </Field>
...
function OperatingHoursPanel({
  ...
  const saveMut = useMutation({
    mutationFn: ({ id, data }: { id?: number; data: object }) =>
      id
        ? sapi('PATCH', `/api/operations/staff/operating-hours/${id}`, data)
        : sapi('POST', `/api/operations/staff/operating-hours`, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/operations/staff/operating-hours'] });
  ...
  const days = DAY_NAMES.map((name, idx) => ({
    name,
    idx,
    row: opHours.find((h) => h.dayOfWeek === idx),
  }));
  return (
        ...
        <tbody className="divide-y divide-slate-100">
          {days.map(({ name, idx, row }) => (
            <HoursRow
              ...
              idx={idx}
              row={row}
              onSave={(data) => saveMut.mutate({ id: row?.id, data })}
...
function HoursRow({
  ...
  return (
    <tr>
      ...
      <td className="px-4 py-2.5">
        <button
          onClick={() => onSave({ dayOfWeek: idx, isOpen, openTime: open, closeTime: close })}
          disabled={saving}
          className="px-2.5 py-1 bg-emerald-600 text-white text-xs rounded hover:bg-emerald-700 disabled:opacity-50"
        >
          Save
        </button>
      </td>
...
function WorkAreasPanel({
  ...
  const saveMut = useMutation({
    mutationFn: (d: z.infer<typeof workAreaSchema> & { id?: number }) =>
      d.id
        ? sapi('PATCH', `/api/operations/staff/work-areas/${d.id}`, d)
        : sapi('POST', `/api/operations/staff/work-areas`, d),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/operations/staff/work-areas'] });
      setModal(null);
      form.reset();
      toast({ title: 'Saved' });
    },
    ...
    <>
      <div className="bg-white border border-slate-200 rounded-lg">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
          ...
          <button
            onClick={() => {
              form.reset({ name: '', sortOrder: workAreas.length });
              setModal({ mode: 'add' });
            }}
            ...
            <tbody className="divide-y divide-slate-100">
              {workAreas.map((a) => (
                <tr key={a.id}>
                  ...
                  <td className="px-4 py-2.5">
                    <span
                      className={`px-1.5 py-0.5 rounded text-xs font-medium ${a.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}
                    >
                  ...
                  <td className="px-4 py-2.5 text-right">
                    <div className="flex gap-1 justify-end">
                      <button
                        onClick={() => {
                          form.reset({ name: a.name, sortOrder: a.sortOrder });
                      ...
                      <button
                        onClick={() => {
                          if (confirm('Delete this work area?')) delMut.mutate(a.id);
                        }}
      ...
      {modal && (
        <Modal
          title={modal.mode === 'add' ? 'Add Work Area' : 'Edit Work Area'}
          onClose={() => setModal(null)}
        >
          <form
            onSubmit={form.handleSubmit((d) => saveMut.mutate({ ...d, id: modal.item?.id }))}
            className="space-y-4"
          >
...
function TemplatesPanel({
  ...
  const saveMut = useMutation({
    mutationFn: (d: z.infer<typeof templateSchema> & { id?: number }) =>
      d.id
        ? sapi('PATCH', `/api/operations/staff/shift-templates/${d.id}`, d)
      ...
      <div className="bg-white border border-slate-200 rounded-lg">
        ...
        {templates.length === 0 ? (
          <div className="p-6 text-center text-xs text-slate-400">
            No shift templates. Add one to speed up roster creation.
          </div>
        ) : (
          <table className="w-full text-xs">
            <thead className="bg-slate-50 border-b border-slate-100">
...
function DailyCleaningTemplatesPanel({
  ...
  const saveMut = useMutation({
    mutationFn: (d: z.infer<typeof cleaningTemplateSchema> & { id?: number }) =>
      d.id
        ? sapi('PATCH', `/api/operations/staff/cleaning/templates/${d.id}`, d)
        : sapi('POST', `/api/operations/staff/cleaning/templates`, { ...d, taskType: 'daily' }),
    onSuccess: () => {
        ...
        >
          ...
          >
            <Field label="Task Name *">
              <input {...form.register('taskName')} className={inputCls} />
            </Field>
