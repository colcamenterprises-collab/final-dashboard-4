...
const taskSchema = z.object({
  ...
  frequency: z.enum(['daily', 'weekly', 'fortnightly', 'monthly', 'quarterly']),
...
export default function DeepCleaningSchedule() {
  ...
  const [editTask, setEditTask] = useState<DeepCleaningTask | null>(null);
  const [statusFilter, setStatusFilter] = useState<'all' | 'pending' | 'overdue' | 'completed'>(
    'all'
  );
  ...
  const createMut = useMutation({
    mutationFn: (d: z.infer<typeof taskSchema>) =>
      sapi('POST', `/api/operations/staff/deep-cleaning`, {
        ...d,
        notes: (d as any).notes || null,
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/operations/staff/deep-cleaning'] });
      setShowModal(false);
      form.reset();
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<z.infer<typeof taskSchema>> }) =>
      sapi('PATCH', `/api/operations/staff/deep-cleaning/${id}`, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/operations/staff/deep-cleaning'] });
      setEditTask(null);
  ...
  const deactivateMut = useMutation({
    mutationFn: (id: number) => sapi('DELETE', `/api/operations/staff/deep-cleaning/${id}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/operations/staff/deep-cleaning'] });
  ...
  const completeMut = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: 'Task completed — next due date scheduled' });
    },
  ...
  const rolloverMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['/api/operations/staff/deep-cleaning'] });
      setCompleteModal(null);
      toast({ title: 'Rolled over — due date extended' });
    },
    onError: () => toast({ title: 'Failed', variant: 'destructive' }),
  });
  ...
  const overdueCount = tasks.filter(
    (t) => t.isActive && t.status !== 'completed' && t.dueDate < today
  ).length;
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Scheduled deep cleans with due dates, rollover tracking and auto-reschedule.
          </p>
        </div>
        <button
          onClick={() => {
                  ...
                  <tr key={task.id} className={isOverdue ? 'bg-red-50' : ''}>
                    <td className="px-4 py-3">
                      <div className="flex items-start gap-1.5">
                        <div>
                          <p
                            className={`font-medium ${isOverdue ? 'text-red-700' : 'text-slate-800'}`}
                          >
                            {task.taskName}
                          </p>
                          {task.rolloverCount > 0 && (
                            <p className="text-xs text-amber-600 mt-0.5">
                              Rolled over {task.rolloverCount}×
                            </p>
                          )}
                        </div>
                        {isOverdue && (
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-1.5 py-0.5 rounded text-xs font-medium ${FREQ_COLORS[task.frequency] ?? 'bg-slate-100 text-slate-600'}`}
                      >
                        {task.frequency}
                      </span>
                    </td>
                    <td
                      className={`px-4 py-3 font-medium ${isOverdue ? 'text-red-600' : 'text-slate-700'}`}
                    >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-1.5 py-0.5 rounded text-xs font-medium ${STATUS_COLORS[task.status] ?? 'bg-slate-100 text-slate-500'}`}
                      >
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          className="px-2 py-1 text-xs border border-slate-300 rounded hover:bg-slate-50"
                        >
                          {task.assignedStaffId ? 'Remove Staff' : 'Assign Staff'}
                        </button>
                        <button
                          onClick={() => deactivateMut.mutate(task.id)}
          ...
          <form onSubmit={form.handleSubmit((d) => createMut.mutate(d))} className="space-y-4">
            <Field label="Task Name *" error={form.formState.errors.taskName?.message}>
              <input
                ...
                className={inputCls}
                placeholder="e.g. Degrease hood filters"
              />
            </Field>
      ...
      {editTask && (
        <Modal title={`Edit Task — ${editTask.taskName}`} onClose={() => setEditTask(null)}>
          <form
            onSubmit={form.handleSubmit((d) =>
              updateMut.mutate({ id: editTask.id, data: { ...d, notes: d.notes || null } as any })
            )}
            className="space-y-4"
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Area *">
                <input {...form.register('areaName')} className={inputCls} />
              </Field>
              <Field label="Frequency">
                <select {...form.register('frequency')} className={inputCls}>
                  <option value="daily">Daily</option>
        ...
        <Modal title={`Action — ${completeModal.taskName}`} onClose={() => setCompleteModal(null)}>
          <div className="space-y-4">
            <p className="text-xs text-slate-600">
              Due: <strong>{fmtDate(completeModal.dueDate)}</strong> · Area:{' '}
              <strong>{completeModal.areaName}</strong>
            </p>
            <div>
              <label className="block space-y-1">
            ...
            <div className="flex gap-2 pt-1">
              <button
                onClick={() => rolloverMut.mutate({ id: completeModal.id, notes: rolloverNote })}
              ...
              <button
                onClick={() => completeMut.mutate({ id: completeModal.id, notes: rolloverNote })}
