...
export default function DailyCleaning() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (salesJson?.ok) {
        ...
        const staff = salesJson.record?.staff || "";
        setManager(staff);
        localStorage.setItem(WORKFLOW_CONTEXT_KEY, JSON.stringify({ shiftId, staff, shiftDate: salesJson.record?.date || new Date().toISOString().slice(0, 10), step: "cleaning", updatedAt: new Date().toISOString() }));
      }
      ...
      if (!salesJson?.ok) {
        try {
          ...
          if (workflow.shiftId === shiftId) {
            if (workflow.staff) setManager(workflow.staff);
            if (workflow.shiftDate) setShiftDate(workflow.shiftDate);
          }
        } catch {}
      }
      setState(next);
  ...
  async function continueToStock() {
    ...
    localStorage.setItem(WORKFLOW_CONTEXT_KEY, JSON.stringify({ shiftId, staff: data.manager || manager, shiftDate, step: "stock", updatedAt: new Date().toISOString() }));
    ...
    window.setTimeout(() => navigate(`/operations/daily-stock?shift=${encodeURIComponent(shiftId)}`, { replace: true }), 1200);
  }
  ...
  if (completion) {
    return <div className="p-4 space-y-4 text-xs max-w-4xl">
      <div className="rounded border border-green-200 bg-green-50 p-4 text-green-900">
        ...
        <p className="mt-4 text-xs">Opening Daily Stock...</p>
        <button className="mt-3 inline-flex rounded bg-emerald-600 px-4 py-2 font-medium text-white" type="button" onClick={() => navigate(`/operations/daily-stock?shift=${encodeURIComponent(shiftId)}`, { replace: true })}>Continue now</button>
      </div>
    </div>;
  }
