...
import { COSTING_NOTES_PREFIX, emptyRecipeForm, fmtMoney, fmtPercent, notesWithoutWorkflowData, parseCostingRows, parseStatus, splitInstructions, toNumber, type MenuCategory, type Recipe, type RecipeIngredientRow } from "./recipeTypes";
...
export default function RecipeEditorPage() {
  ...
  const costRows = useMemo(() => ingredients.map((row) => { const unitCost = toNumber(row.manualOverrideUnitCost); const qty = toNumber(row.quantityUsed); return { row, unitCost, qty, lineCost: unitCost !== null && qty !== null ? unitCost * qty : null }; }), [ingredients]);
  const validRows = costRows.filter((entry) => entry.row.name.trim() && entry.row.unitUsed.trim() && entry.unitCost !== null && entry.qty !== null && entry.qty > 0);
  const missingRows = costRows.filter((entry) => entry.row.name.trim() && (!entry.row.unitUsed.trim() || entry.unitCost === null || entry.qty === null || entry.qty <= 0));
  ...
  return <div className="mx-auto max-w-5xl space-y-4 p-4">
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><Link to="/menu/recipes" className="text-xs underline">Back to Recipes</Link><h1 className="text-xl font-semibold">{isNewRecipe ? "Add New Recipe" : recipe!.name}</h1><Badge variant={form.status === "Live" ? "default" : "outline"}>{form.status}</Badge></div><button onClick={() => saveMutation.mutate()} disabled={!form.name || !form.category || saveMutation.isPending} className="rounded-lg bg-black px-4 py-2 text-xs text-white disabled:opacity-40">{saveMutation.isPending ? "Saving..." : "Save Recipe"}</button></div>
    <section className="space-y-3 rounded-lg border bg-white p-4"><div><h2 className="text-sm font-semibold">Recipe Details</h2><p className="mt-1 text-xs text-slate-500">Recipes are private development records. Saving here does not publish an item to the POS.</p></div><div className="grid grid-cols-1 gap-3 md:grid-cols-2"><Input placeholder="Recipe title" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /><select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} className="rounded border px-2 py-2 text-xs"><option value="">Menu category</option>{categories.map((category) => <option key={category.id} value={category.name}>{category.name}</option>)}</select><Input placeholder="Description" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} className="md:col-span-2" /><input type="file" accept="image/png,image/jpeg,image/webp" onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; const data = new FormData(); data.append("image", file); const res = await fetch("/api/upload/menu-item-image", { method: "POST", body: data }); const uploaded = await res.json(); if (!res.ok) throw new Error(uploaded?.error || "Image upload failed"); setForm({ ...form, imageUrl: uploaded.imageUrl || uploaded.url }); }} className="w-full rounded-md border border-slate-200 px-3 py-2 text-xs file:mr-3 file:rounded file:border-0 file:bg-slate-100 file:px-2 file:py-1 file:text-xs md:col-span-2" /><label className="block text-xs font-medium">Yield quantity<Input type="number" min="0.01" step="any" value={form.yieldQuantity} onChange={(event) => setForm({ ...form, yieldQuantity: event.target.value })} className="mt-1" /></label><label className="block text-xs font-medium">Yield unit<Input value={form.yieldUnit} onChange={(event) => setForm({ ...form, yieldUnit: event.target.value })} className="mt-1" /></label><select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as typeof form.status })} className="rounded border px-2 py-2 text-xs"><option>Draft</option><option>Tested</option><option>Approved</option><option>Archived</option></select></div><div className="flex h-36 w-36 items-center justify-center overflow-hidden rounded-lg border bg-slate-100 text-xs text-slate-500">{form.imageUrl ? <img src={form.imageUrl} alt={form.name} className="h-full w-full object-cover" /> : "No image"}</div></section>
    ...
    {missingRows.length > 0 && blockerText(`${missingRows.length} ingredient row(s) need a quantity, unit and unit cost.`)}
