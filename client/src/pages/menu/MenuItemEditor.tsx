...
export default function MenuItemEditor({ item, categories, recipes, modifierGroups, onClose }: Props) {
  ...
  const [draft, setDraft] = useState({ name: item.name || "", categoryId: resolvedCategoryId, description: item.description || "", imageUrl: item.imageUrl || "", clearImage: false, price: String(item.basePrice ?? item.price ?? 0), recipeId: item.recipeId ? String(item.recipeId) : "", modifierGroupIds: initiallyLinked, displayOrder: String(item.displayOrder ?? item.sortOrder ?? 0), isActive: item.isActive !== false });
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={onClose}>
    <aside className="h-full w-full max-w-xl overflow-y-auto bg-white p-5 shadow-xl" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="space-y-4">
        ...
        <section className="rounded-xl border p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-medium">Menu item image</p><p className="text-[11px] text-slate-500">This image feeds the POS catalogue and live POS.</p></div>{draft.imageUrl && <button type="button" onClick={removeImage} className="rounded-lg border border-red-300 px-3 py-1.5 text-xs text-red-700">Remove image</button>}</div><div className="mt-3 flex items-start gap-3"><div className="flex h-28 w-28 shrink-0 items-center justify-center overflow-hidden rounded-xl border bg-slate-100 text-xs text-slate-500">{draft.imageUrl ? <img src={draft.imageUrl} alt={draft.name} className="h-full w-full object-contain" /> : "No image"}</div><div className="min-w-0 flex-1 space-y-2"><input ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/webp" disabled={uploading} onChange={(event) => { setUploadError(""); setUploadMessage(""); setSelectedFile(event.target.files?.[0] ?? null); }} className="w-full rounded-md border border-slate-200 px-3 py-2 text-xs file:mr-3 file:rounded file:border-0 file:bg-slate-100 file:px-2 file:py-1 file:text-xs" />{selectedFile && <div className="rounded-lg bg-slate-50 px-3 py-2 text-xs"><strong>Selected:</strong> {selectedFile.name}</div>}<button type="button" disabled={!selectedFile || uploading} onClick={() => void uploadImage()} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold disabled:opacity-40">{uploading ? "Uploading and saving image…" : "Upload selected image"}</button><p className="text-[11px] text-slate-500">Use a 600 × 600 PNG, JPG or WebP.</p>{uploadMessage && <p className="rounded-lg bg-emerald-50 px-3 py-2 text-xs text-emerald-800">{uploadMessage}</p>}{uploadError && <p className="text-xs text-red-700">{uploadError}</p>}</div></div></section>
        ...
        <label className="block text-xs font-medium">Linked recipe<select value={draft.recipeId} onChange={(event) => setDraft({ ...draft, recipeId: event.target.value })} className="mt-1 w-full rounded-md border px-3 py-2 text-sm"><option value="">No recipe linked</option>{recipes.map((recipe) => <option key={recipe.id} value={recipe.id}>{recipe.name}</option>)}</select></label>
        ...
        <button disabled={Boolean(saveBlockedReason) || save.isPending} onClick={() => save.mutate()} className="w-full rounded-lg bg-black px-4 py-3 text-sm font-semibold text-white disabled:opacity-40">{save.isPending ? "Saving menu item…" : "Save menu item"}</button>
      </div>
    </aside>
  </div>;
}
