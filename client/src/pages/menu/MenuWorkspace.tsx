...
export default function MenuWorkspace() {
  ...
  const updateMenuItemMutation = useMutation({ mutationFn: (item: MenuItem) => apiRequest("/api/menu-v3/items/update", { method: "POST", body: JSON.stringify({ id: item.id, name: item.name, categoryId: item.categoryId, description: item.description, basePrice: item.basePrice ?? item.price, imageUrl: item.imageUrl, isActive: item.isActive !== false, posEnabled: item.posEnabled !== false, onlineEnabled: item.onlineEnabled ?? item.isOnlineEnabled ?? true, sortOrder: item.sortOrder ?? item.displayOrder ?? 0 }) }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/menu-v3/items"] }); setEditingItemId(null); setItemDraft(null); } });
  const publishMenuItemMutation = useMutation({ mutationFn: (id: string) => apiRequest(`/api/pos/menu-items/${id}/publish`, { method: "POST" }), onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/pos/menu"] }) });
  const uploadMenuItemImage = async (file: File) => {
    ...
    setItemDraft((current) => current ? { ...current, imageUrl: body.imageUrl } : current);
  };
  ...
  const editingItem = editingItemId ? itemDraft ?? items.find((item) => item.id === editingItemId) ?? null : null;
    ...
    {activeTab === "items" && <div className="space-y-5">
      ...
      <div className="space-y-6">{itemGroups.map(([categoryName, categoryItems]) => {
        ...
        return <section key={categoryName} className="space-y-3"><button type="button" onClick={() => setCollapsedCategories((current) => ({ ...current, [categoryName]: !collapsed }))} className="flex w-full items-center justify-between gap-3 text-left"><h2 className="text-2xl font-bold text-slate-950">{categoryName}</h2><span className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-emerald-50 text-emerald-800">{collapsed ? <ChevronDown className="h-5 w-5" /> : <ChevronUp className="h-5 w-5" />}</span></button>{!collapsed && <div className="space-y-3">{categoryItems.map((item) => {
          ...
          return <article key={item.id} role="button" tabIndex={0} onClick={() => { setEditingItemId(item.id); setItemDraft({ ...item }); }} onKeyDown={(event) => { if (event.key === "Enter") { setEditingItemId(item.id); setItemDraft({ ...item }); } }} className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm transition hover:border-emerald-300"><div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-slate-100 text-slate-400">{item.imageUrl ? <img src={item.imageUrl} alt={item.name || "Menu item"} className="h-full w-full object-cover" /> : <ImageIcon className="h-7 w-7" />}</div><div className="min-w-0 flex-1"><h3 className="text-base font-semibold text-slate-950">{item.name || "UNMAPPED"}</h3><p className="mt-1 text-sm text-slate-500">{item.description || "—"}</p><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500"><span className="font-mono text-sm text-slate-900">{price === null ? "UNMAPPED" : fmtMoney(price)}</span><span>Linked recipe: {recipe?.name || "No recipe linked"}</span>{groupsForItem.length > 0 && <span>Modifiers: {groupsForItem.map((group) => group.name).join(", ")}</span>}</div></div><button type="button" aria-label={`${available ? "Mark unavailable" : "Mark available"} ${item.name || "menu item"}`} onClick={(event) => { event.stopPropagation(); toggleMenuItemMutation.mutate({ id: item.id, isActive: !available }); }} className={`relative h-8 w-14 shrink-0 rounded-full transition ${available ? "bg-emerald-500" : "bg-slate-300"}`}><span className={`absolute top-1 h-6 w-6 rounded-full bg-white transition ${available ? "left-7" : "left-1"}`} /></button></article>;
        })}</div>}</section>;
      })}</div>
      {editingItem && <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={() => { setEditingItemId(null); setItemDraft(null); }}><aside className="h-full w-full max-w-xl overflow-y-auto bg-white p-5 shadow-xl" onClick={(event) => event.stopPropagation()}><div className="mb-4 flex items-start justify-between gap-3"><div><p className="text-xs text-slate-500">Menu item editor</p><h2 className="text-xl font-semibold">{editingItem.name || "UNMAPPED"}</h2></div><button className="rounded-lg border px-3 py-1.5 text-xs" onClick={() => { setEditingItemId(null); setItemDraft(null); }}>Close</button></div><div className="space-y-3"><label className="block text-xs font-medium">Item name<Input value={editingItem.name || ""} onChange={(e) => setItemDraft({ ...editingItem, name: e.target.value })} className="mt-1" /></label><label className="block text-xs font-medium">Category<Input value={itemCategoryName(editingItem, categoryMap)} readOnly className="mt-1" /></label><label className="block text-xs font-medium">Image<div className="mt-1 flex items-center gap-3"><div className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-xl bg-slate-100 text-slate-400">{editingItem.imageUrl ? <img src={editingItem.imageUrl} alt={editingItem.name} className="h-full w-full object-cover" /> : <ImageIcon className="h-8 w-8" />}</div><div className="flex-1 space-y-2"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => { const file = e.target.files?.[0]; if (file) uploadMenuItemImage(file).catch((error) => window.alert(error.message)); }} className="block w-full text-xs" /><Input value={editingItem.imageUrl || ""} onChange={(e) => setItemDraft({ ...editingItem, imageUrl: e.target.value })} /></div></div></label><label className="block text-xs font-medium">Description<textarea value={editingItem.description || ""} onChange={(e) => setItemDraft({ ...editingItem, description: e.target.value })} className="mt-1 min-h-24 w-full rounded-md border px-3 py-2 text-sm" /></label><label className="block text-xs font-medium">RRP / customer price<Input type="number" value={String(editingItem.basePrice ?? editingItem.price ?? "")} onChange={(e) => setItemDraft({ ...editingItem, basePrice: e.target.value })} className="mt-1 font-mono" /></label><div className="flex items-center justify-between rounded-lg border p-3 text-sm"><span>Available/customer-visible</span><button type="button" onClick={() => setItemDraft({ ...editingItem, isActive: !itemAvailable(editingItem) })} className={`relative h-8 w-14 rounded-full transition ${itemAvailable(editingItem) ? "bg-emerald-500" : "bg-slate-300"}`}><span className={`absolute top-1 h-6 w-6 rounded-full bg-white transition ${itemAvailable(editingItem) ? "left-7" : "left-1"}`} /></button></div><div className="rounded-lg border p-3 text-sm"><p className="text-xs font-medium text-slate-500">Linked recipe</p><p>{(editingItem.recipeId ? recipes.find((r) => r.id === editingItem.recipeId) : editingItem.recipes?.[0]?.recipe ?? recipes.find((r) => r.name?.toLowerCase() === editingItem.name?.toLowerCase()))?.name || "No recipe linked"}</p></div><div className="grid grid-cols-2 gap-2"><button onClick={() => updateMenuItemMutation.mutate(editingItem)} disabled={updateMenuItemMutation.isPending} className="rounded-lg bg-black px-4 py-2 text-sm font-medium text-white disabled:opacity-50">Save changes</button><button onClick={() => publishMenuItemMutation.mutate(editingItem.id)} disabled={publishMenuItemMutation.isPending || updateMenuItemMutation.isPending} className="rounded-lg bg-amber-400 px-4 py-2 text-sm font-semibold text-black disabled:opacity-50">{publishMenuItemMutation.isPending ? "Publishing…" : "Save & publish to POS"}</button></div><p className="text-xs text-slate-500">Save changes first, then publish. The POS receives this item’s current name, price, availability and image.</p></div></aside></div>}
    </div>}
