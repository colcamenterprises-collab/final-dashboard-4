...
function Panel({title,subtitle,children,className=""}:{title:string;subtitle?:string;children:React.ReactNode;className?:string}) { return <section className={`rounded-[28px] border border-white/10 bg-[#18181b] p-5 shadow-[0_20px_55px_rgba(0,0,0,.24)] ${className}`}><div><h2 className="text-base font-black text-white">{title}</h2>{subtitle?<p className="mt-1 text-xs text-zinc-500">{subtitle}</p>:null}</div><div className="mt-5">{children}</div></section>; }

function HorizontalBar({label,value,max,meta,color}:{label:string;value:number;max:number;meta:string;color:string}) { const width=max>0?Math.max(3,Math.min(100,value/max*100)):0; return <div className="space-y-2"><div className="flex justify-between gap-3 text-xs"><span className="truncate font-bold text-zinc-200">{label}</span><span className="shrink-0 text-zinc-500">{meta}</span></div><div className="h-2 overflow-hidden rounded-full bg-white/5"><div className={`h-full rounded-full ${color}`} style={{width:`${width}%`}} /></div></div>; }
...
export default function ReportingOverview(){
 ...
 return <div className="min-h-screen rounded-[32px] bg-[#09090b] p-4 text-white md:p-6">
  <header className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-xs font-black uppercase tracking-[.25em] text-blue-400">Restaurant intelligence</p><h1 className="mt-2 text-3xl font-black tracking-tight md:text-4xl">Reporting Overview</h1><p className="mt-2 text-sm text-zinc-500">Fast decisions from one trusted sales ledger.</p></div><div className="rounded-2xl bg-white p-2 text-slate-900"><ExactDateTimeRange value={range} onChange={setRange} timezoneLabel="Venue time · Asia/Bangkok"/></div></header>
  {query.isLoading?<div className="rounded-3xl border border-white/10 bg-white/5 p-10 text-sm text-zinc-400">Loading reporting data…</div>:null}
  {query.isError?<div className="rounded-3xl border border-red-500/30 bg-red-500/10 p-5 text-sm font-bold text-red-300">{(query.error as Error).message}</div>:null}
  ...
  <div className="mt-5 grid gap-5 xl:grid-cols-[1.65fr_1fr]">
    <Panel title="Hourly Sales" subtitle={`One bar per trading hour · ${range.fromTime} opening to ${range.toTime} closing`}>
      {hourly.length?<ResponsiveContainer width="100%" height={300}><BarChart data={hourly} margin={{top:18,right:4,left:0,bottom:0}}><defs><linearGradient id="hourlySales" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#60a5fa"/><stop offset="100%" stopColor="#4f46e5"/></linearGradient></defs><CartesianGrid vertical={false} stroke="#27272a" strokeDasharray="3 3"/><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{fill:"#a1a1aa",fontSize:11,fontWeight:700}}/><YAxis axisLine={false} tickLine={false} width={50} tick={{fill:"#71717a",fontSize:10}} tickFormatter={compactMoney}/><Tooltip cursor={{fill:"rgba(255,255,255,.04)"}} contentStyle={{background:"#09090b",border:"1px solid #3f3f46",borderRadius:16,color:"white"}} formatter={(value:number,_name:string,entry:any)=>[`${money(value)} · ${entry.payload.orders} orders`,"Sales"]}/><Bar dataKey="sales" fill="url(#hourlySales)" radius={[10,10,3,3]} maxBarSize={58}/></BarChart></ResponsiveContainer>:<p className="text-sm text-zinc-500">No sales in this shift window.</p>}
    </Panel>
    <Panel title="Payment Mix" subtitle="Net sales by payment channel"><div className="space-y-3">{Object.entries(paymentGroups).map(([label,amount])=>{const Style=paymentStyle[label];const Icon=Style.icon;const pct=data.netSales>0?amount/data.netSales*100:0;return <div key={label} className="flex items-center gap-3 rounded-2xl bg-white/[.04] p-3"><span className={`rounded-xl p-2 text-slate-950 ${Style.color}`}><Icon className="h-4 w-4"/></span><div className="min-w-0 flex-1"><div className="flex justify-between text-xs"><span className="font-bold text-zinc-200">{label}</span><span className="font-black text-white">{money(amount)}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/5"><div className={`h-full rounded-full ${Style.color}`} style={{width:`${Math.max(pct?3:0,pct)}%`}}/></div></div><span className="w-10 text-right text-[10px] font-bold text-zinc-500">{pct.toFixed(0)}%</span></div>})}</div></Panel>
  </div>
  ...
  <footer className="mt-5 flex flex-col gap-2 rounded-2xl border border-white/10 bg-white/[.03] p-4 text-[11px] text-zinc-500 sm:flex-row sm:items-center sm:justify-between"><span>Sources: {(query.data?.sourcesIncluded||[]).join(" + ")||"No transactions"}</span><span className="flex items-center gap-1"><Clock3 className="h-3 w-3"/>{query.data?.filters.fromInstant} → {query.data?.filters.toInstant}</span></footer></>:null}
 </div>;
}
