...
type Item = { item_key:string; item_name:string; sku?:string; category?:string; quantity:number; gross_sales:number; discounts:number; refunds:number; net_sales:number; costed_net_sales:number; uncosted_net_sales:number; known_cost_of_goods:number; known_gross_profit:number; profit_covered_net_sales:number; cost_of_goods:number|null; gross_profit:number|null; margin_pct:number|null; sources:string[] };
...
const rowCoverage=(row:Item)=>Number(row.net_sales||0)!==0?Number(row.costed_net_sales||0)/Number(row.net_sales||0)*100:null;
...
export default function SalesByItem(){
  ...
  const totals=useMemo(()=>{
    ...
    const net=rows.reduce((sum,row)=>sum+Number(row.net_sales||0),0);
    const cogsRows=rows.filter(row=>String(row.category||"").toLowerCase()!=="promotions");
    const cogsScopeNet=cogsRows.reduce((sum,row)=>sum+Number(row.net_sales||0),0);
    const costedNet=cogsRows.reduce((sum,row)=>sum+Number(row.costed_net_sales||0),0);
    const uncostedNet=cogsRows.reduce((sum,row)=>sum+Number(row.uncosted_net_sales||0),0);
    const knownCogs=cogsRows.reduce((sum,row)=>sum+Number(row.known_cost_of_goods||0),0);
    const knownGrossProfit=cogsRows.reduce((sum,row)=>sum+Number(row.known_gross_profit||0),0);
    const profitCoveredNet=cogsRows.reduce((sum,row)=>sum+Number(row.profit_covered_net_sales||0),0);
    const fullCogs=cogsRows.reduce((sum,row)=>sum+Number(row.cost_of_goods||0),0);
    const fullGrossProfit=cogsRows.reduce((sum,row)=>sum+Number(row.gross_profit||0),0);
    const fullyCosted=cogsRows.length>0&&cogsRows.every(row=>row.cost_of_goods!=null&&row.gross_profit!=null);
    const fullMarginPct=fullyCosted&&cogsScopeNet!==0?fullGrossProfit/cogsScopeNet*100:null;
    const averageCogsPct=fullyCosted&&cogsScopeNet!==0?fullCogs/cogsScopeNet*100:costedNet!==0?knownCogs/costedNet*100:null;
    return {quantity,net,cogsScopeNet,costedNet,uncostedNet,knownCogs,knownGrossProfit,profitCoveredNet,fullCogs,fullGrossProfit,coveragePct:cogsScopeNet!==0?costedNet/cogsScopeNet*100:null,fullyCosted,fullMarginPct,averageCogsPct};
  },[rows]);
  ...
  const exportItemCsv=()=>{
    const header=["Item","SKU","Category","Qty","Gross Sales","Discounts","Refunds","Net Sales","Costed Net Sales","Uncosted Net Sales","Known COGS","Known Gross Profit","Cost Coverage %","Full COGS","Full Gross Profit","Full Margin %","Sources"];
    const body=rows.map(row=>[row.item_name,row.sku,row.category,row.quantity,row.gross_sales,row.discounts,row.refunds,row.net_sales,row.costed_net_sales,row.uncosted_net_sales,row.known_cost_of_goods,row.known_gross_profit,rowCoverage(row),row.cost_of_goods,row.gross_profit,row.margin_pct,sourceLabel(row.sources)].map(csv).join(","));
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    ...
    {tab==="items"?<><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Metric label="Items sold" value={totals.quantity.toLocaleString()}/><Metric label="Net sales" value={money(totals.net)}/><Metric label={totals.fullyCosted?"COGS":"Known COGS"} value={money(totals.fullyCosted?totals.fullCogs:totals.knownCogs)}/><Metric label="Average COGS %" value={percent(totals.averageCogsPct)}/><Metric label={totals.fullyCosted?"Gross margin":"Cost coverage"} value={totals.fullyCosted?percent(totals.fullMarginPct):percent(totals.coveragePct)}/></div>{!totals.fullyCosted&&rows.length?<div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs font-semibold text-amber-950">Known COGS and Average COGS % use only sales with verified sale-time item, set-component and modifier cost snapshots. Full COGS, gross profit and margin remain withheld until coverage reaches 100%.</div>:null}</>:null}
...
function ItemsTable({rows}:{rows:Item[]}){return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[1450px] text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3 text-left">Item</th><th className="px-4 py-3 text-left">Category</th><th className="px-4 py-3 text-left">Source</th><th className="px-4 py-3 text-right">Qty</th><th className="px-4 py-3 text-right">Gross</th><th className="px-4 py-3 text-right">Discount</th><th className="px-4 py-3 text-right">Refund</th><th className="px-4 py-3 text-right">Net Sales</th><th className="px-4 py-3 text-right">Costed Sales</th><th className="px-4 py-3 text-right">Uncosted Sales</th><th className="px-4 py-3 text-right">Known COGS</th><th className="px-4 py-3 text-right">Known GP</th><th className="px-4 py-3 text-right">Coverage</th><th className="px-4 py-3 text-right">Full Margin</th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map(row=><tr key={row.item_key} className="hover:bg-slate-50"><td className="px-4 py-3"><div className="font-black">{row.item_name}</div>{row.sku?<div className="text-[10px] text-slate-400">{row.sku}</div>:null}</td><td className="px-4 py-3">{row.category||"Other"}</td><td className="px-4 py-3 text-xs">{sourceLabel(row.sources)}</td><td className="px-4 py-3 text-right font-bold">{Number(row.quantity).toLocaleString()}</td><td className="px-4 py-3 text-right">{money(row.gross_sales)}</td><td className="px-4 py-3 text-right">{money(row.discounts)}</td><td className="px-4 py-3 text-right">{money(row.refunds)}</td><td className="px-4 py-3 text-right font-black">{money(row.net_sales)}</td><td className="px-4 py-3 text-right">{money(row.costed_net_sales)}</td><td className="px-4 py-3 text-right">{money(row.uncosted_net_sales)}</td><td className="px-4 py-3 text-right font-bold">{money(row.known_cost_of_goods)}</td><td className="px-4 py-3 text-right">{money(row.known_gross_profit)}</td><td className="px-4 py-3 text-right font-bold">{percent(rowCoverage(row))}</td><td className="px-4 py-3 text-right">{percent(row.margin_pct)}</td></tr>)}</tbody></table></div></div>;}
