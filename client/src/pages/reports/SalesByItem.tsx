...
type Component = { type: "Modifier" | "Option" | "Upsell"; group: string; name: string; qtySold: number; revenue: number; source?: string };
type Included = { name: string; category: string; qtySold: number; source?: string };
...
type Data = {
  ...
  receipts?: Receipt[];
  summary: { grossSales: number; receiptCount: number; lineItemCount?: number; historicalUnits?: number; liveUnits?: number };
...
type ItemColumnKey = "source" | "item" | "sku" | "category" | "qty" | "refQty" | "gross" | "refunds" | "discount" | "net" | "cogs" | "profit" | "margin" | "taxes" | "avg";
...
const ALL_COLUMNS: { key: ItemColumnKey; label: string; defaultVisible: boolean }[] = [
  ...
  { key: "cogs", label: "Cost of Goods", defaultVisible: false },
...
const STORAGE_KEY = "sbb:item-sales:visible-columns:v2";
...
export default function SalesByItem() {
  ...
  const { data, isLoading, isError, error } = useQuery<Data>({
    queryKey: ["sales-by-item-v3", params],
    queryFn: async () => {
  ...
  const showHistoricalArchive = () => {
    setFromDate("2026-01-01");
    setFromTime("17:00");
    setToDate("2026-08-08");
    setToTime("03:00");
    setTab("items");
  };
  ...
  const valueFor = (item: Item, key: ItemColumnKey) => {
    ...
    if (key === "qty") return item.qtySold;
    if (key === "refQty") return item.itemsRefunded;
    if (key === "gross") return item.grossSales;
    if (key === "refunds") return item.refunds;
    if (key === "discount") return item.discounts;
    if (key === "net") return item.netSales;
    if (key === "cogs") return item.costOfGoods;
    if (key === "profit") return item.grossProfit;
    if (key === "margin") return item.marginPct == null ? "" : `${item.marginPct}%`;
    if (key === "taxes") return item.taxes;
  ...
  const exportCsv = () => {
    ...
    if (tab === "items") {
      ...
      rows = [["Component", "Category", "Qty Used", "Source"], ...included.map(x => [x.name, x.category, String(x.qtySold), x.source || "SBB POS"])];
      filename = `included-components-${fromDate}-${toDate}.csv`;
    } else {
      const source = tab === "upsells" ? upsells : modifiers;
      rows = [["Type", "Group", "Selection", "Qty", "Value", "Source"], ...source.map(x => [x.type, x.group, x.name, String(x.qtySold), String(x.revenue), x.source || "SBB POS"])];
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
      <PageTitle title="Item Sales" meta="Unified historical archive + live SBB POS reporting" />
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          {tab === "items" && <div className="relative">
            ...
            {showColumns && <div className="absolute right-0 z-20 mt-2 w-64 rounded-xl border bg-white p-3 shadow-xl">
              <p className="mb-2 text-[10px] font-black uppercase text-slate-500">Visible columns</p>
              <div className="grid grid-cols-1 gap-2">{ALL_COLUMNS.map(c => <label key={c.key} className="flex items-center gap-2 text-xs"><input type="checkbox" checked={isVisible(c.key)} onChange={() => toggleColumn(c.key)} />{c.label}</label>)}</div>
            </div>}
          </div>}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-xs text-blue-900">
        <b>Live source of truth:</b> SBB POS from 8 Aug 2026 17:00. Loyverse is retired and retained only as a read-only aggregate archive for 1 Jan through the 7 Aug shift. Historical aggregates never create synthetic receipts.
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
        ...
        <div className="rounded-xl border bg-white p-4"><p className="text-[10px] font-bold uppercase text-slate-500">Modifier/Upsell value</p><p className="text-xl font-black">{money(modifierRevenue)}</p><p className="text-[10px] text-slate-400">SBB POS selections</p></div>
        <div className={`rounded-xl border p-4 ${isError ? "border-red-200 bg-red-50" : reconciled ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}>
          <p className="text-[10px] font-bold uppercase text-slate-500">Reconciliation</p>
          <p className={`text-xl font-black ${isError ? "text-red-700" : reconciled ? "text-emerald-700" : "text-amber-700"}`}>{isError ? "ERROR" : reconciled ? "PASS" : "CHECK"}</p>
          <p className="text-[10px] text-slate-500">live receipts + archive checksum</p>
        </div>
      </div>

      <div className="flex flex-wrap gap-2 border-b pb-2">
        {([["items", "Items"], ["modifiers", "Modifiers & Options"], ["upsells", "Upsells"], ["included", "Included Components"]] as [Tab, string][]).map(([key, label]) => (
          <button key={key} onClick={() => setTab(key)} className={`rounded-lg px-3 py-2 text-xs font-bold ${tab === key ? "bg-slate-900 text-white" : "border bg-white text-slate-600"}`}>{label}</button>
        ))}
      </div>

      {isLoading && <div className="rounded-2xl border bg-white p-8 text-center text-sm text-slate-500">Loading item sales…</div>}
      {isError && <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-red-700">Could not load item sales. {error instanceof Error ? error.message : ""}</div>}
      ...
      {!isLoading && !isError && tab === "items" && <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div className="border-b px-4 py-3"><h2 className="font-black">Items Sold</h2><p className="text-xs text-slate-500">Historical and live records remain source-labelled. Discontinued historical products remain visible permanently.</p></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm">
          <thead className="bg-slate-50 text-[11px] uppercase text-slate-500"><tr>{ALL_COLUMNS.filter(c => isVisible(c.key)).map(c => <th key={c.key} className={`px-4 py-3 ${["source", "item", "sku", "category"].includes(c.key) ? "text-left" : "text-right"}`}>{c.label}</th>)}</tr></thead>
          <tbody className="divide-y">{items.map((item, index) => <tr key={`${item.source}-${item.sku}-${index}`}>
            {ALL_COLUMNS.filter(c => isVisible(c.key)).map(c => <td key={c.key} className={`px-4 py-3 ${["source", "item", "sku", "category"].includes(c.key) ? "text-left" : "text-right"} ${c.key === "item" || c.key === "net" ? "font-bold" : ""}`}>{renderItemCell(item, c.key)}</td>)}
          </tr>)}</tbody>
        </table></div>
      </section>}

      {!isLoading && !isError && tab === "modifiers" && <section className="overflow-hidden rounded-2xl border bg-white"><div className="border-b px-4 py-3"><h2 className="font-black">Modifiers & Options</h2></div><div className="overflow-x-auto"><table className="w-full min-w-[650px] text-sm"><thead className="bg-slate-50 text-xs"><tr><th className="px-4 py-3 text-left">Type</th><th className="px-4 py-3 text-left">Group</th><th className="px-4 py-3 text-left">Selection</th><th className="px-4 py-3 text-right">Qty</th><th className="px-4 py-3 text-right">Value</th></tr></thead><tbody className="divide-y">{modifiers.map((x, i) => <tr key={`${x.group}-${x.name}-${i}`}><td className="px-4 py-3">{x.type}</td><td className="px-4 py-3">{x.group}</td><td className="px-4 py-3 font-bold">{x.name}</td><td className="px-4 py-3 text-right">{x.qtySold}</td><td className="px-4 py-3 text-right">{money(x.revenue)}</td></tr>)}</tbody></table></div></section>}

      {!isLoading && !isError && tab === "upsells" && <section className="overflow-hidden rounded-2xl border bg-white"><div className="border-b px-4 py-3"><h2 className="font-black">Upsells</h2></div><div className="overflow-x-auto"><table className="w-full min-w-[650px] text-sm"><thead className="bg-slate-50 text-xs"><tr><th className="px-4 py-3 text-left">Group</th><th className="px-4 py-3 text-left">Upsell</th><th className="px-4 py-3 text-right">Qty</th><th className="px-4 py-3 text-right">Value</th></tr></thead><tbody className="divide-y">{upsells.map((x, i) => <tr key={`${x.group}-${x.name}-${i}`}><td className="px-4 py-3">{x.group}</td><td className="px-4 py-3 font-bold">{x.name}</td><td className="px-4 py-3 text-right">{x.qtySold}</td><td className="px-4 py-3 text-right">{money(x.revenue)}</td></tr>)}</tbody></table></div></section>}

      {!isLoading && !isError && tab === "included" && <section className="overflow-hidden rounded-2xl border bg-white"><div className="border-b px-4 py-3"><h2 className="font-black">Included Components</h2><p className="text-xs text-slate-500">Zero-additional-revenue components retained for later recipe and stock usage calculations.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[550px] text-sm"><thead className="bg-slate-50 text-xs"><tr><th className="px-4 py-3 text-left">Component</th><th className="px-4 py-3 text-left">Category</th><th className="px-4 py-3 text-right">Qty Used</th></tr></thead><tbody className="divide-y">{included.map((x, i) => <tr key={`${x.category}-${x.name}-${i}`}><td className="px-4 py-3 font-bold">{x.name}</td><td className="px-4 py-3">{x.category}</td><td className="px-4 py-3 text-right">{x.qtySold}</td></tr>)}</tbody></table></div></section>}
    </div>
  );
}
