...
export default function AdminSettings() {
  ...
  const field = (label: string, key: string, type = "text", hint?: string) => <label className="block text-sm font-medium text-neutral-700">{label}<input type={type} value={settings[key] ?? ""} onChange={(e) => setSettings({ ...settings, [key]: type === "number" ? e.target.value : e.target.value })} className="mt-1 w-full rounded-xl border border-neutral-300 bg-white px-3 py-2.5 outline-none focus:border-neutral-950" />{hint && <span className="mt-1 block text-xs font-normal text-neutral-500">{hint}</span>}</label>;
  ...
  return <main className="mx-auto max-w-5xl space-y-6 p-4 md:p-6">
    ...
    <section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
      <h2 className="text-lg font-bold text-neutral-950">Restaurant</h2>
