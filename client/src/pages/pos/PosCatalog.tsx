...
export default function PosCatalog() {
  ...
  const save = useMutation({ mutationFn: (item: Item) => apiRequest(`/api/pos/catalog/items/${item.id}`, { method: "PATCH", body: JSON.stringify(item) }), onSuccess: () => { invalidate(); setEditing(null); } });
  const create = useMutation({ mutationFn: (item: Partial<Item>) => apiRequest("/api/pos/catalog/items", { method: "POST", body: JSON.stringify(item) }), onSuccess: () => { invalidate(); setNewItem(null); setCreating(false); } });
