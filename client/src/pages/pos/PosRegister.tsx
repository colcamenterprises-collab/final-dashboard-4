  ...
  return <main className="h-dvh overflow-hidden bg-[#fffdf4] text-[#171717]">
    ...
    <div className="grid h-[calc(100dvh-70px)] grid-cols-[minmax(0,1fr)_348px] overflow-hidden">
      ...
      <aside className="m-3 ml-0 flex min-w-0 flex-col overflow-hidden rounded-[26px] border border-[#eee9d9] bg-white shadow-[0_10px_30px_rgba(38,31,7,0.08)]">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5">{cart.length === 0 ? <div className="grid h-full place-items-center text-center"><div><div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-[#fff6c9] text-xl">+</div><p className="mt-3 text-sm font-bold text-zinc-500">{language === "th" ? "เพิ่มสินค้าเพื่อเริ่มออเดอร์" : "Add items to start an order"}</p></div></div> : cart.map((line,index) => <div key={`${line.id}-${index}`} className="border-b border-[#f1eee4] py-3"><div className="flex justify-between gap-2 text-sm font-bold"><span className="leading-5">{line.quantity} × {label(line)}</span><span>{thb(lineTotal(line))}</span></div>{(line.modifiers || []).map(modifier => <p key={modifier.id} className="mt-1 text-xs font-medium text-[#15945c]">+ {label(modifier)} · {thb(modifier.price_delta)}</p>)}{line.set_upgrade && <div className="mt-1 text-xs font-bold text-[#856a00]"><p>SET UPGRADE +฿80</p><p>1 × French Fries</p><p>1 × {drinks.find(drink => drink.id === line.set_drink_menu_item_id)?.name_en}</p></div>}{isBurger(line) && <input list="burger-request-suggestions" value={line.notes || ""} onChange={event=>setCart(current=>current.map((item,i)=>i === index ? {...item,notes:event.target.value} : item))} className="mt-2 w-full rounded-xl border border-[#e9e4d5] bg-[#fffdf8] px-3 py-2 text-xs outline-none focus:border-[#ffd400]" placeholder={language === "th" ? "คำขอ เช่น ไม่ใส่ชีส" : "Item request, e.g. No cheese"}/>}</div>)}</div>
