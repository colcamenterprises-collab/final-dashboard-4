...
const speakTicket = (ticket:string, language:"en"|"th") => { if (!("speechSynthesis" in window)) return; window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(language === "th" ? `ออเดอร์ ${ticket} ส่งเข้าครัวแล้ว` : `Order ${ticket} sent to kitchen`); utterance.lang = language === "th" ? "th-TH" : "en-US"; window.speechSynthesis.speak(utterance); };
...
export default function PosRegister() {
  ...
  useEffect(() => {
    fetch(`/api/pos/menu?price_mode=${mode}`,{credentials:"include"})
      .then(r => r.json()).then(x => { setItems((x.data || []).map((item:Item) => ({...item,active_price:Number(item.active_price || 0)}))); setCart([]); })
  ...
  const charge = async () => {
    try {
      ...
      setNotice(`${body.data.ticket_number} sent to kitchen`); speakTicket(body.data.ticket_number,language); setCart([]); setCash(""); refreshOrderNumber(); window.setTimeout(()=>setNotice(""),4000);
    } catch (error:any) { setNotice(error.message); }
  };
  ...
  return <main className="h-dvh overflow-hidden bg-[#fffdf4] text-[#171717]">
    ...
    <div className="grid h-[calc(100dvh-70px)] grid-cols-[minmax(0,1fr)_348px] overflow-hidden">
      ...
      <aside className="m-3 ml-0 flex min-w-0 flex-col overflow-hidden rounded-[26px] border border-[#eee9d9] bg-white shadow-[0_10px_30px_rgba(38,31,7,0.08)]"><div className="flex items-center justify-between border-b border-[#eee9d9] px-5 py-4"><div><h2 className="text-xl font-black">{language === "th" ? "ออเดอร์ปัจจุบัน" : "Current order"}</h2><p className="mt-1 text-xs font-bold text-zinc-400">{language === "th" ? "ออเดอร์" : "Order"} · {orderNumber}</p></div><button onClick={()=>setCart([])} className="rounded-xl px-2 py-1 text-sm font-semibold text-red-600 hover:bg-red-50">{language === "th" ? "ล้าง" : "Clear"}</button></div><div className="min-h-0 flex-1 overflow-y-auto px-5">{cart.length === 0 ? <div className="grid h-full place-items-center text-center"><div><div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-[#fff6c9] text-xl">+</div><p className="mt-3 text-sm font-bold text-zinc-500">{language === "th" ? "เพิ่มสินค้าเพื่อเริ่มออเดอร์" : "Add items to start an order"}</p></div></div> : cart.map((line,index) => <div key={`${line.id}-${index}`} className="border-b border-[#f1eee4] py-3"><div className="flex justify-between gap-2 text-sm font-bold"><span className="leading-5">{line.quantity} × {label(line)}</span><span>{thb(lineTotal(line))}</span></div>{(line.modifiers || []).map(modifier => <p key={modifier.id} className="mt-1 text-xs font-medium text-[#15945c]">+ {label(modifier)} · {thb(modifier.price_delta)}</p>)}{line.set_upgrade && <div className="mt-1 text-xs font-bold text-[#856a00]"><p>SET UPGRADE +฿80</p><p>1 × French Fries</p><p>1 × {drinks.find(drink => drink.id === line.set_drink_menu_item_id)?.name_en}</p></div>}{isBurger(line) && <input list="burger-request-suggestions" value={line.notes || ""} onChange={event=>setCart(current=>current.map((item,i)=>i === index ? {...item,notes:event.target.value} : item))} className="mt-2 w-full rounded-xl border border-[#e9e4d5] bg-[#fffdf8] px-3 py-2 text-xs outline-none focus:border-[#ffd400]" placeholder={language === "th" ? "คำขอ เช่น ไม่ใส่ชีส" : "Item request, e.g. No cheese"}/>}</div>)}</div><datalist id="burger-request-suggestions"><option value="No cheese"/><option value="No tomato"/><option value="No salad"/><option value="No onions"/><option value="No pickles"/><option value="No jalapenos"/><option value="No burger sauce"/><option value="No meat"/><option value="No bun"/></datalist><div className="border-t border-[#eee9d9] bg-[#fffefa] p-5"><div className="flex items-end justify-between"><span className="text-lg font-black">{language === "th" ? "ยอดรวม" : "Total"}</span><span className="text-3xl font-black">{thb(total)}</span></div>{mode === "direct" && payment === "cash" && <><input inputMode="decimal" value={cash} onChange={event=>setCash(event.target.value)} placeholder={language === "th" ? "รับเงินสด" : "Cash received"} className="mt-3 w-full rounded-xl border border-[#e9e4d5] bg-white px-3 py-3 text-sm outline-none focus:border-[#ffd400]"/><div className="mt-2 flex justify-between text-sm font-bold"><span>{language === "th" ? "เงินทอน" : "Change"}</span><span>{thb(change)}</span></div></>}<div className="mt-4 grid grid-cols-3 gap-2">{["cash","manual_qr_transfer","grab"].map(method => <button key={method} disabled={mode === "grab" && method !== "grab"} onClick={()=>setPayment(method)} className={`rounded-xl py-3 text-xs font-black transition ${payment === method ? "bg-[#171717] text-white" : "bg-[#f4f2eb] text-zinc-600 hover:bg-[#ebe7d8]"}`}>{method === "manual_qr_transfer" ? "QR" : method}</button>)}</div><button disabled={!cart.length} onClick={charge} className="mt-3 w-full rounded-xl bg-[#ffd400] py-4 text-base font-black text-black shadow-[0_7px_0_#d7ae00] transition hover:bg-[#ffe042] active:translate-y-0.5 active:shadow-[0_4px_0_#d7ae00] disabled:cursor-not-allowed disabled:opacity-40">{language === "th" ? "ชำระ" : "Charge"} {thb(total)}</button></div></aside>
    </div>
