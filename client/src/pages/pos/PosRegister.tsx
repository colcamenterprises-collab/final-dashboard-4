...
export default function PosRegister() {
  ...
  useEffect(() => {
    ...
    categories.forEach(category => {
      const element = document.getElementById(category.replaceAll(" ","-"));
  ...
  return <main className="h-dvh overflow-hidden bg-[#fffdf4] text-[#171717]">
    <header className="flex h-[70px] items-center justify-between bg-[#111111] px-5 text-white shadow-lg">
      <div className="flex items-center gap-4">
        ...
        <nav className="hidden gap-1 lg:flex"><span className="rounded-xl bg-white/10 px-4 py-2 text-sm font-semibold">POS</span><a href="/pos/kitchen" className="rounded-xl px-4 py-2 text-sm text-zinc-300 hover:bg-white/10">Kitchen</a><a href="/pos/display" className="rounded-xl px-4 py-2 text-sm text-zinc-300 hover:bg-white/10">Ticket display</a><a href="/pos/shifts" className="rounded-xl px-4 py-2 text-sm text-zinc-300 hover:bg-white/10">Shift</a></nav>
      </div>
      <div className="flex items-center gap-2">
        <button className="hidden rounded-xl bg-white/10 px-3 py-2 text-xs font-semibold sm:block" onClick={openDiscountManager}>Discounts</button>
        ...
        <button className={`rounded-xl px-4 py-2 text-sm font-bold ${mode === "direct" ? "bg-white text-black" : "bg-white/10"}`} onClick={()=>setMode("direct")}>Counter</button>
    ...
    <div className="grid h-[calc(100dvh-70px)] grid-cols-[minmax(0,1fr)_348px] overflow-hidden">
      ...
      <aside className="m-3 ml-0 flex min-w-0 flex-col overflow-hidden rounded-[26px] border border-[#eee9d9] bg-white shadow-[0_10px_30px_rgba(38,31,7,0.08)]">
        <div className="flex items-center justify-between border-b border-[#eee9d9] px-5 py-4"><div><h2 className="text-xl font-black">{language === "th" ? "ออเดอร์ปัจจุบัน" : "Current order"}</h2><p className="mt-1 text-xs font-bold text-zinc-400">{language === "th" ? "ออเดอร์" : "Order"} · {orderNumber}</p></div><button onClick={()=>setCart([])} className="rounded-xl px-2 py-1 text-sm font-semibold text-red-600 hover:bg-red-50">{language === "th" ? "ล้าง" : "Clear"}</button></div>
        <div className="min-h-[220px] max-h-[42vh] flex-none overflow-y-auto px-5">{cart.length === 0 ? <div className="grid min-h-[220px] place-items-center text-center"><div><div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-[#fff6c9] text-xl">+</div><p className="mt-3 text-sm font-bold text-zinc-500">{language === "th" ? "เพิ่มสินค้าเพื่อเริ่มออเดอร์" : "Add items to start an order"}</p></div></div> : <div className="py-2">{cart.map((line,index) => <div key={`${line.id}-${index}`} className="border-b border-dashed border-[#d9d2c0] py-3"><div className="flex justify-between gap-2 text-sm font-bold"><span className="leading-5">{line.quantity} × {label(line)}</span><span>{thb(lineTotal(line))}</span></div>{(line.modifiers || []).map(modifier => <p key={modifier.id} className="mt-1 text-xs font-medium text-[#15945c]">+ {label(modifier)} · {thb(modifier.price_delta)}</p>)}{line.set_upgrade && <div className="mt-1 text-xs font-bold text-[#856a00]"><p>SET UPGRADE +฿80</p><p>1 × French Fries</p><p>1 × {drinks.find(drink => drink.id === line.set_drink_menu_item_id)?.name_en}</p></div>}{isBurger(line) && <input list="burger-request-suggestions" value={line.notes || ""} onChange={event=>setCart(current=>current.map((item,i)=>i === index ? {...item,notes:event.target.value} : item))} className="mt-2 w-full rounded-xl border border-[#e9e4d5] bg-[#fffdf8] px-3 py-2 text-xs outline-none focus:border-[#ffd400]" placeholder={language === "th" ? "คำขอ เช่น ไม่ใส่ชีส" : "Item request, e.g. No cheese"}/>}</div>)}</div>}</div>
        ...
        <div className="border-t border-[#eee9d9] bg-[#fffefa] p-5">
          <div className="flex items-end justify-between"><span className="text-lg font-black">{language === "th" ? "ยอดรวม" : "Total"}</span><span className="text-3xl font-black">{thb(total)}</span></div>
          ...
          <label className="mt-3 block text-xs font-black text-zinc-600">Discount code<select value={selectedDiscount} onChange={event=>setSelectedDiscount(event.target.value)} className="mt-1 w-full rounded-xl border border-[#e9e4d5] bg-white px-3 py-3 text-sm outline-none focus:border-[#ffd400]"><option value="">No discount</option>{discounts.filter(discount=>discount.code !== "OWNER100").map(discount => <option key={discount.id} value={discount.code}>{discount.name}</option>)}{discounts.some(discount=>discount.code === "OWNER100") && <option disabled>────────────────</option>}{discounts.filter(discount=>discount.code === "OWNER100").map(discount => <option key={discount.id} value={discount.code}>{discount.name}</option>)}</select></label>
          {mode === "grab" && <div className="mt-4 space-y-2 rounded-2xl border border-[#ffd400] bg-[#fff9d9] p-3"><p className="text-xs font-black text-[#856a00]">GRAB ORDER DETAILS</p><label className="block text-xs font-bold">Enter the Order Number - GF ________<input value={grabOrderNumber} onChange={event=>setGrabOrderNumber(event.target.value.toUpperCase())} placeholder="GF-ABCDE" className="mt-1 w-full rounded-xl border border-[#e9d678] bg-white px-3 py-2 text-sm outline-none focus:border-black"/></label><label className="block text-xs font-bold">Customer name<input value={grabCustomerName} onChange={event=>setGrabCustomerName(event.target.value)} className="mt-1 w-full rounded-xl border border-[#e9d678] bg-white px-3 py-2 text-sm outline-none focus:border-black"/></label><label className="block text-xs font-bold">Mobile number<input inputMode="tel" value={grabCustomerMobile} onChange={event=>setGrabCustomerMobile(event.target.value)} className="mt-1 w-full rounded-xl border border-[#e9d678] bg-white px-3 py-2 text-sm outline-none focus:border-black"/></label></div>}
          {mode === "direct" && payment === "cash" && <><input inputMode="decimal" value={cash} onChange={event=>setCash(event.target.value)} placeholder={language === "th" ? "รับเงินสด" : "Cash received"} className="mt-3 w-full rounded-xl border border-[#e9e4d5] bg-white px-3 py-3 text-sm outline-none focus:border-[#ffd400]"/><div className="mt-2 flex justify-between text-sm font-bold"><span>{language === "th" ? "เงินทอน" : "Change"}</span><span>{thb(change)}</span></div></>}
          ...
          <button disabled={!cart.length} onClick={startCheckout} className="mt-3 w-full rounded-xl bg-[#ffd400] py-4 text-base font-black text-black shadow-[0_7px_0_#d7ae00] transition hover:bg-[#ffe042] active:translate-y-0.5 active:shadow-[0_4px_0_#d7ae00] disabled:cursor-not-allowed disabled:opacity-40">{language === "th" ? "ดำเนินการชำระเงิน" : "Continue"} {thb(total)}</button>
        </div>
      </aside>
    </div>
