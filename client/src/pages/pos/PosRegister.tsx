...
type Line = Item & { quantity:number; set_upgrade?:boolean; set_drink_menu_item_id?:string; modifiers?:Modifier[] };
  ...
  const charge = async () => {
    try {
      const response = await fetch("/api/pos/orders",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({
        ...
        items:cart.map(x => ({menu_item_id:x.id,quantity:x.quantity,set_upgrade:mode === "direct" && !!x.set_upgrade,set_drink_menu_item_id:x.set_drink_menu_item_id,modifier_ids:(x.modifiers || []).map(m => m.id)}))
      })});
      const body = await response.json();
