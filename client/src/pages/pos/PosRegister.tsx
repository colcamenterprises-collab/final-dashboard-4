  ...
  return <main className="h-dvh overflow-hidden bg-[#fffdf4] text-[#171717]">
    ...
    <div className="relative grid h-[calc(100dvh-70px)] grid-cols-[minmax(0,1fr)_348px] overflow-hidden">
      ...
      <section className="relative z-10 min-w-0 overflow-y-auto pb-10">
        ...
        {categories.map((category,categoryIndex) => <section id={category.name_en.replaceAll(" ","-")} data-category={category.name_en} key={category.name_en} className={`mb-12 scroll-mt-28 rounded-[28px] p-5 ${categoryIndex % 2 === 0 ? "bg-[#fff9e8]" : "bg-[#f7f5ee]"}`}><div className="mb-5 flex items-center gap-3"><h2 className="text-xl font-black">{categoryLabel(category)}</h2><span className="h-px flex-1 bg-[#ded8c7]"/></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">{items.filter(x => x.category_name === category.name_en).map(item => <button key={item.id} onClick={()=>startItem(item)} className="group relative min-h-[250px] overflow-hidden rounded-[22px] border border-[#eee9d9] bg-white p-3 text-left shadow-[0_7px_22px_rgba(38,31,7,0.055)] transition hover:-translate-y-1 hover:border-[#ffd400] hover:shadow-[0_13px_28px_rgba(38,31,7,0.11)]"><div className="flex h-[145px] items-center justify-center bg-transparent">{(item.image_url || isBurger(item)) && <img src={item.image_url || burgerImage} alt="" className="h-[140px] w-full max-w-[220px] bg-transparent object-contain drop-shadow-[0_10px_10px_rgba(39,27,8,0.14)]"/>}</div><p className="mt-1 line-clamp-2 min-h-10 text-[13px] font-extrabold leading-5">{label(item)}</p><p className="mt-2 text-center text-3xl font-black leading-none">{thb(item.active_price)}</p><span className="mt-2 grid h-8 w-full place-items-center rounded-full bg-[#ffd400] text-lg font-black leading-none text-black transition group-hover:bg-black group-hover:text-white">+</span></button>)}</div></section>)}</div>
      </section>
