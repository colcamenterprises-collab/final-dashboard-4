  ...
  return <main className="h-dvh overflow-hidden bg-white text-[#171717]">
    <header className="flex h-[70px] items-center justify-between bg-[#111111] px-5 text-white shadow-lg">
      <div className="flex items-center gap-4">
        ...
        <nav className="hidden gap-1 lg:flex"><span className="rounded-xl bg-white/10 px-4 py-2 text-sm font-semibold">{ui.pos}</span><a href="/pos/kitchen" className="rounded-xl px-4 py-2 text-sm text-zinc-300 hover:bg-white/10">{ui.kitchen}</a><a href="/pos/display" className="rounded-xl px-4 py-2 text-sm text-zinc-300 hover:bg-white/10">{ui.display}</a><a href="/pos/shifts" className="rounded-xl px-4 py-2 text-sm text-zinc-300 hover:bg-white/10">{ui.shift}</a><a href="/" className="rounded-xl px-4 py-2 text-sm font-semibold text-[#ffd400] hover:bg-white/10">Dashboard</a></nav>
      </div>
      <div className="flex items-center gap-2">
        ...
        <button className={`rounded-xl px-4 py-2 text-sm font-bold ${mode === "direct" ? "bg-white text-black" : "bg-white/10"}`} onClick={()=>{setMode("direct");setCart([]);setGrabOrderNumber("");setGrabDiscountAmount("");setSelectedDiscount("")}}>{ui.counter}</button>
        <button className={`rounded-xl px-4 py-2 text-sm font-bold ${mode === "grab" ? "bg-[#ffd400] text-black" : "bg-white/10"}`} onClick={()=>{setMode("grab");setCart([]);setGrabOrderNumber("");setGrabDiscountAmount("");setSelectedDiscount("")}}>Grab</button>
      </div>
    </header>
