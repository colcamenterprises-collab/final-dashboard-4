      ...
      <div className="flex flex-wrap gap-2">
        {PRESETS.map((preset) => (
          <button
            ...
            className="rounded-lg border bg-white px-3 py-2 text-xs font-bold hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {preset.label}
          </button>
        ))}
        ...
        <section key={group}>
          <div className="mb-2 flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              disabled={disabled}
              className="text-xs font-semibold text-slate-500 hover:text-black disabled:opacity-40"
              onClick={() => {
              ...
              return (
                <label
                  ...
                  className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 transition ${
                    checked ? "border-black bg-slate-50" : "bg-white hover:bg-slate-50"
                ...
                >
                  <input
                    ...
                    checked={checked}
                    onChange={(event) => onChange({ ...value, [permission.key]: event.target.checked })}
                  />
                  <span>
...
export default function StaffAccess() {
  ...
  const usersQuery = useQuery<{ users: User[] }>({
    queryKey: ["pin-auth-staff"],
    queryFn: () => api("/api/pin-auth/staff"),
  });
  ...
  const create = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setMessage(`Access created for ${data.user.username}`);
      queryClient.invalidateQueries({ queryKey: ["pin-auth-staff"] });
    },
  ...
  const updateAccess = useMutation({
    mutationFn: () => {
      ...
      return api(`/api/pin-auth/staff/${editing.id}`, {
        method: "PUT",
        body: JSON.stringify({
          role: editRole,
          permissions: editRole === "owner" ? {} : editPermissions,
        }),
      });
    },
    onSuccess: () => {
      setMessage("Role and permissions updated. The user must sign out and back in for changes to take effect.");
      setEditing(null);
      queryClient.invalidateQueries({ queryKey: ["pin-auth-staff"] });
    },
    onError: (error: Error) => setMessage(error.message),
  ...
  const toggle = useMutation({
    mutationFn: (user: User) =>
      api(`/api/pin-auth/staff/${user.id}`, {
        method: "PUT",
  ...
  const reset = useMutation({
    mutationFn: () =>
      api(`/api/pin-auth/staff/${resetFor!.id}/pin`, {
        method: "PATCH",
        body: JSON.stringify({ pin: newPassword }),
      }),
    onSuccess: () => {
      setMessage("Password / PIN reset");
      setResetFor(null);
      setNewPassword("");
    },
    onError: (error: Error) => setMessage(error.message),
  ...
  const remove = useMutation({
    ...
    onSuccess: () => {
      setMessage("Staff login permanently deleted");
      setDeleteFor(null);
      queryClient.invalidateQueries({ queryKey: ["pin-auth-staff"] });
    },
    onError: (error: Error) => setMessage(error.message),
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-2 md:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-black">Staff Access & Permissions</h1>
          </div>
          <p className="mt-1 max-w-3xl text-sm text-slate-500">
            Create owner and staff accounts, assign a role, and control exactly which sections of the restaurant system each person can access.
          </p>
        </div>
        <button
          onClick={() => setShowCreate((value) => !value)}
          className="flex items-center gap-2 rounded-xl bg-black px-4 py-2 text-sm font-bold text-white"
        >
      ...
      {message && (
        <div className="rounded-xl border bg-white px-4 py-3 text-sm">
          {message}
          <button className="float-right text-slate-400" onClick={() => setMessage("")}>
            ×
          </button>
        </div>
      )}
      ...
      {owners.length === 0 && (
        <div className="rounded-xl border border-red-300 bg-red-50 p-4 font-bold text-red-800">
          No active owner account is visible. Do not deactivate the final owner.
        </div>
      )}
      ...
      {showCreate && (
        <div className="space-y-5 rounded-2xl border bg-white p-5">
          <div>
            <h2 className="text-lg font-black">Create user access</h2>
            <p className="text-sm text-slate-500">Owners receive full access automatically. Other roles can be customised below.</p>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <input
              className="rounded-lg border p-3"
              placeholder="Username (optional; generated if blank)"
              value={form.username}
            ...
            <input
              ...
              className="rounded-lg border p-3"
              placeholder="Password / PIN (4–72 characters)"
              value={form.password}
          ...
          {form.role !== "owner" && (
            <div className="rounded-xl border">
              <button
                type="button"
                onClick={() => setShowCreatePermissions((value) => !value)}
                className="flex w-full items-center justify-between gap-3 p-4 text-left"
              >
                <span>
                  <span className="block font-black">Custom access permissions</span>
                  <span className="text-xs text-slate-500">Tick the exact areas this person can use.</span>
                </span>
          ...
          {form.role === "owner" && (
            <div className="flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
              ...
              <div>
                <div className="font-black">Owner = full system access</div>
                <div className="mt-1 text-xs">Owner accounts are not restricted by the individual permission matrix.</div>
              </div>
            </div>
          )}
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              disabled={!form.name || form.password.length < 4 || create.isPending}
              onClick={() => create.mutate()}
              className="rounded-xl bg-[#FFD400] px-5 py-3 font-black disabled:opacity-40"
            >
            ...
            {users.map((user) => {
              ...
              return (
                <tr key={user.id}>
                  ...
                  <td className="p-4">
                    {user.role === "owner" ? (
                      <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-800">Full access</span>
                    ) : (
                      <span className="text-xs text-slate-600">{grantedCount} custom permissions</span>
                    )}
                  </td>
                  <td className="p-4">
                    {user.active ? <span className="text-emerald-700">Active</span> : <span className="text-red-600">Disabled</span>}
                  </td>
                  <td className="p-4">
                    <div className="flex justify-end gap-2">
                      <button
                      ...
                      <button
                        disabled={user.role === "owner" && user.active && owners.length === 1}
                        onClick={() => toggle.mutate(user)}
                        className="flex items-center gap-1 rounded-lg border px-3 py-2 disabled:cursor-not-allowed disabled:opacity-30"
                      ...
                      <button
                        ...
                        className="rounded-lg border border-red-200 p-2 text-red-700 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-30"
                        title="Delete user"
                      >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4">
          <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-6">
            <div className="flex flex-wrap items-start justify-between gap-3 border-b pb-4">
              <div>
                <h2 className="text-2xl font-black">Access for {editing.name}</h2>
                <p className="mt-1 text-sm text-slate-500">Change role/title and select exactly which sections this user can access.</p>
              </div>
              <button className="text-2xl text-slate-400" onClick={() => setEditing(null)}>
                ×
              </button>
            </div>

            <div className="my-5 max-w-sm">
              <label className="mb-1 block text-xs font-black uppercase tracking-wide text-slate-500">Role / title</label>
              <select className="w-full rounded-lg border p-3" value={editRole} onChange={(event) => setEditRole(event.target.value)}>
                <option value="staff">Staff</option>
                <option value="cashier">Cashier</option>
                <option value="kitchen_staff">Kitchen staff</option>
                <option value="manager">Manager</option>
            ...
            {editRole === "owner" ? (
              <div className="flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
                ...
                <div>
                  <div className="font-black">Owner = full system access</div>
                  <div className="mt-1 text-xs">Saving this user as an owner grants unrestricted access to the system.</div>
                </div>
              </div>
            ) : (
            ...
            <div className="mt-6 flex justify-end gap-2 border-t pt-4">
              <button className="rounded-lg border px-4 py-2 font-bold" onClick={() => setEditing(null)}>
                Cancel
              </button>
              <button
                disabled={updateAccess.isPending}
                className="rounded-lg bg-black px-5 py-2 font-bold text-white disabled:opacity-40"
      ...
      {resetFor && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6">
            <h2 className="text-xl font-black">Reset access for {resetFor.name}</h2>
            <p className="mt-1 text-sm text-slate-500">Username: {resetFor.username}</p>
            <input
              autoFocus
              type="password"
              autoComplete="new-password"
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button className="rounded-lg border px-4 py-2" onClick={() => setResetFor(null)}>
                Cancel
              </button>
              <button
                disabled={newPassword.length < 4 || reset.isPending}
                className="flex items-center gap-2 rounded-lg bg-black px-4 py-2 font-bold text-white disabled:opacity-40"
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6">
            <div className="flex items-center gap-3 text-red-700">
              ...
              <h2 className="text-xl font-black">Permanently delete login?</h2>
            </div>
            <p className="mt-3 text-sm text-slate-600">
              This will permanently remove <strong>{deleteFor.name}</strong> ({deleteFor.username || "no username"}) from Staff Access. This cannot be undone.
            </p>
            <div className="mt-5 flex justify-end gap-2">
              <button className="rounded-lg border px-4 py-2" onClick={() => setDeleteFor(null)}>
                Cancel
              </button>
              <button
                disabled={remove.isPending}
                className="flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2 font-bold text-white disabled:opacity-40"
